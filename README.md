...
> v0.7.7 起：长篇正文的字数与交付终于只有一套机器口径。`storyctl.py` 在写作中做一次纯测量 checkpoint，完成后统一检查长度和 blocking quality；欠字不再为了凑数自动加戏，超字只做一次不新增语义的净删压缩，仍不合格就交给用户决定。缺少合法「字数目标」不再回退到 3000。新增工作区级作者记忆，确认过的稳定写作偏好可以跨会话延续，但与单本剧情追踪隔离；Codex 生成封面优先使用内置 ImageGen。部署器同时修复了 reference 自复制和嵌套残留问题。**本版 `agents_version` 为 26**，已部署项目需更新技能包、重新运行 `/story-setup` 并新开会话。
>
