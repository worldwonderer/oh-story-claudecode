...
> **Antigravity 支持预览**：`story-setup` 可把 14 个 Skills、7 个 custom agents、Always-On Rule 和 workspace Hooks 完整部署到项目 `.agents/`。部署器不修改 `~/.gemini/`，不依赖全局目录或 symlink；`.agents/hooks.json` 只替换顶层 `oh-story` 管理组并保留用户组。本契约使用 `agents_version: 30`，部署后需新开 Antigravity conversation；IDE 与交互式 `agy` 建议分别试用。

> **候选重构 — 六维拆书**：长篇拆文保留黄金三章，将全文阶段收敛为五列机械索引、20 列结构块与三个自包含全局分析，并新增断点守卫。升级后需重跑 `/story-setup`、新开会话；候选契约 `agents_version` 为 30。
>
