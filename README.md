...
**正文外包给 Gemini Flash（可选，默认关闭）：** 大纲、细纲、追踪在 Claude Code / Codex 里做没问题，只想换个模型写正文时用这个。开启后**只有正文写作那一步**交给外部 CLI，其余全部留在当前 CLI：细纲、追踪、去AI味与字数闸门、`compress-once` 一个都不变。需要 `agy` 在 PATH 中并已登录 Google 账号——**不需要安装 Antigravity IDE**，那个独立二进制就够。`story-setup` 会先预检（约 5 秒），缺 CLI 或未登录就保持关闭并说明原因，绝不影响任何现有功能；写作中途预检失败会静默回落到本地写手，不中断写作。`agy` 自身在 Windows 上是否可用尚未实测——不可用时预检返回「未安装」，外包保持关闭，写作照常。代价要知情：实测 3000 字草稿约 120 秒，比本地慢；且草稿普遍超长（实测 +26% 到 +64%），会多走一次压缩；委派方只读、正文由宿主落盘，所以不需要给它任何写权限。委派方自估字数不可信（实测自估 3042、实际 4819），一律以宿主口径重数，超长走既有 `compress-once`。
