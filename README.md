...
本版重构长篇与短篇拆书 skill，改动集中在拆书 skill 内部，更新 skill 包后即生效。

- **长篇拆书**：「快速 / 深度」双模式合并为单一拆解管道。跑完黄金三章会先产出 `快速预览.md` 并询问是否继续全量拆解，确认后从中断处续跑，不重跑已完成阶段。
- **短篇拆书**：去掉「标准 / 精细」双档，统一为单一全量拆解；管道阶段术语与长篇对齐。
- 两个拆书 skill 的质量阈值、分块策略收敛到唯一权威文件，文档不再各处重复。
