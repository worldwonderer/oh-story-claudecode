...
> **Antigravity support preview:** `story-setup` can deploy all 14 Skills, 7 custom agents, an Always-On Rule, and workspace Hooks into the project's `.agents/` tree. The deployer does not modify `~/.gemini/`, depend on global directories, or require symlink discovery. In `.agents/hooks.json`, it replaces only the top-level `oh-story` group and preserves user groups. This contract keeps `agents_version: 29`; open a fresh Antigravity conversation after deployment and smoke-test the IDE and interactive `agy` separately.
...
**Delegating prose to Gemini Flash (optional, off by default):** Use this when outlines, beat sheets, and tracking work fine in Claude Code / Codex and you only want a different model for the prose itself. When enabled, **only the prose-writing step** goes to an external CLI; everything else stays in your current CLI — beat sheets, tracking, the de-slop and word-count gates, and `compress-once` are all unchanged. It needs `agy` on PATH and signed in to a Google account — **the Antigravity IDE is not required**, the standalone binary is enough. `story-setup` runs a preflight first (about 5 seconds); if the CLI is missing or not signed in it stays off and says why, and never affects existing functionality. A preflight failure mid-writing falls back silently to the local writer rather than interrupting the run. Whether `agy` itself runs on Windows is untested here — if it is unavailable the preflight reports it as missing, delegation stays off, and writing proceeds normally. Know the cost: a 3,000-character draft measured about 120 seconds, slower than writing locally, and drafts still run long (+13% to +44% in testing), so an extra compression pass is normal. The delegate is read-only and the host writes the file, so it needs no write permission at all. Its self-reported length is not trustworthy (3,042 self-reported vs 4,819 actual in testing), so the host always recounts and routes over-length drafts through the existing `compress-once`.
...
**Reasonix users:** Current support is Skills + a native plugin manifest. Reasonix natively scans project skill roots (`.agents/skills` etc., a symlink to `skills/`) and discovers all 14 skills — verify with `reasonix doctor capabilities`; you can also `reasonix plugin install` via the root `reasonix-plugin.json`. When `story-setup` targets `target_cli=reasonix`, it copies the skills into project `skills/` and writes a Reasonix `AGENTS.md`; hooks/custom agents are intentionally deferred, so skills needing specialist agents fall back to solo/direct. If Windows symlinks are disabled, use the native plugin instead.
