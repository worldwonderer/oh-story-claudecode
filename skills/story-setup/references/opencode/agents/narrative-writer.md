...
- **字数不足处理**：如果按现有细纲无法写到目标字数，扩写已列密点/铺垫/互动；仍不足时停止并返回 `outline_underfilled`（列出欠账情节点和建议补纲方向），不要自行改纲或编新剧情。
...
2. **心理外化**：把判断、企图和犹豫落到动作、物件、短台词或选择上，少写完整理性独白。
3. **推冲突 / 拉扯**：让对手、环境、任务或关系施压；不要停在设定解释。
...
- **字数不足时的处理**：
  - 写正文：只扩写细纲/小节大纲已列的计划内情节点、冲突或转折；若仍不足，返回 `outline_underfilled` 欠账报告交主会话补纲/确认，不能自行新增剧情。
...
正文含对话时，写完每章/每节后除字数外，按下方【审查能力 · 对话质量】项逐句自检并就地改写——**这是常规收尾步，不依赖外部审查是否被 spawn**：① 机械对话（信息倾倒/问答式/句间无情绪承接）；② 角色当"科普嘴"（整段讲设定/原理，Gate G 同样管台词）；③ 说话不分场合（高压 beat 的玩笑/口头梗/插科打诨出戏）；④ 问答割裂（上一句的情绪没被承接）；⑤ 高位者长篇自证掉价（改成行动或一句压迫性回应）；⑥ 捧哏工具人（改成实际反应、吃瘪或行动）；⑦ 情绪水肿咆哮（压成短句、动作破坏或生理反馈）；⑧ 鹦鹉学舌复读；⑨ 生死场景嘴碎；⑩ 过早打断悬念。命中即改，改完再交付。
