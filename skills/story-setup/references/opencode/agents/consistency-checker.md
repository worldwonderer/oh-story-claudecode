...
3. 读 `追踪/_tracking-state.json` 取 `schema_version` / `state_revision` / `last_committed_chapter`，并与 `追踪/上下文.md` 头部的 `状态修订：{N}` 比对（本 agent 无 Bash，不跑 `tracking_commit.py`；逐字级派生视图校验由调用方在主会话跑 `check` 负责）；对不上时先把派生视图不可信列为 S1，不继续用它们作一致性结论
