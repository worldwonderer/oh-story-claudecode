...
1. 先将查询角色名中的正则特殊字符转义。用 `Grep "^## {转义后的角色名}$" 追踪/角色状态.md` 定位文件上方该角色的状态小节，再从这个标题行 `Read` 到下一个 `## ` 标题（或文件末尾），取得身份、能力、关系和公众形象的基准值；再限定在 `## 逐章更新记录` 表内，用竖线分列，只保留**角色列与查询角色名完全相同**的数据行。角色列精确匹配式可写成 `^\|[^|]*\|[[:space:]]*{转义后的角色名}[[:space:]]*\|`；排除表头和分隔线，不要用不限列的 `Grep "{角色名}"`，也**不要把角色名只出现在第四列的关系记录算到该角色名下**。不能只看带角色名的标题行，否则会漏掉小节内的基准值
2. **按维度分别合并**：将记录按身份、能力、关系、形象（含“公众形象”）分组，每个维度取章节号最大的记录，同章有多条时取文件中最后一条；**关系记录还要按关系对象分别合并**，从 `与{对象}：{当前关系}` 解析对象，例如“与沈栀”和“与林深”分别取各自最新值，不得互相覆盖。旧记录无法确定关系对象时，只有正文或状态小节能唯一确定对象才合并，否则在 `gaps` 标记待确认。某维度或某个关系对象没有更新记录时保留状态小节中的基准值，两处都没有才标为未知。**不得把某一条单维度变更当成角色的完整当前状态**
3. `Glob 设定/角色/{name}*.md` -> `Read` 角色设定文件（基础人设：出身/性格/口头禅；**不含剧情变化**，不用它覆盖第 2 步得到的剧情状态）
4. `Grep 正文/ "{角色名}"` -> 找到所有出场章节，`Read` 最近 1-2 章出场正文的相关段落（用行号定位）
5. 汇总返回：当前状态 = 第 2 步合并后的各维度状态 + 最近出场正文互证；两者矛盾时以正文为准并在返回里标注哪个维度有冲突
...
1. 指定了 ID 或关键词 -> `Grep 追踪/伏笔.md` 只取命中行；同一 ID 有多行时（含 `## 逐章更新记录` 的追加行），当前状态以**最后一条命中**为准。要完整清单（foreshadow_list）才 `Read` 整个文件——文件可能长到上百 KB，读取留在本 agent 上下文内，返回只给筛选后的条目
...
1. 给了章节范围 -> `Grep "第.*章" 追踪/时间线.md` 取范围内的行（含 `## 逐章更新记录` 的追加行）；要全书时间线才 `Read` 整个文件，读取留在本 agent 上下文内
2. 按章节范围筛选
3. 如需更多细节 -> `Read` 对应正文
4. 返回时间节点列表
...
1. `Read 追踪/上下文.md` -> 写作状态摘要，日更所需状态（在场角色 / 活跃伏笔 / 近三章速记 / 长期约束 / 下一章必做事项）全在这里。如不存在，`Glob 正文/第*.md` 扫描最大章节号推断下一章编号
2. 调用方必须按 workflow-daily 统计普通事项，并传入 `select-pending-items.py --limit 6 --ordinary-count {普通事项数}` 的 JSON；本 agent 不信任状态摘要索引，也不 `Glob` 待处理事项目录。按 JSON 中的精确路径 `Read`，最多读取 6 个小文件；用 JSON 的 `record_id` 去重，按目标章、来源章和序号作数字排序后，把事项与路径作为 `pending_items` 返回。目录文件是未结清跨章事项的完整依据，摘要只是其中最早事项的副本；本 agent 保持只读，不删除文件。JSON 缺失、`invalid_count > 0` 或 `due_overflow` 为 true 时在 `gaps` 标记阻塞。这是 context_load 唯一允许读取的额外追踪内容。
3. `Read 大纲/细纲_第{N}章.md` -> 本章写作计划
4. 从细纲提取角色名 -> `Read 设定/角色/{name}.md`
5. `Read 正文/第{N-1}章_*.md` -> 最新一章（衔接用）
6. **仅当**调用方在查询参数里明确给出伏笔 ID 或角色名，且状态摘要里查不到时，才 `Grep` 对应历史记录文件（`追踪/伏笔.md` / `追踪/角色状态.md` / `追踪/时间线.md` / `追踪/逐章记录/`）并只取最后 3 条命中。默认不读这些文件，它们已长到上百 KB
7. 汇总为"写作上下文包"

> 除第 2 步中按需读取的少量单项待处理文件外，历史记录文件全文读取只在 `foreshadow_status` / `character_status` / `timeline` 这三个按需查询 query_type 里发生，且读取停留在本 agent 的上下文内、只把按固定字段整理的摘要返给调用方——这样主会话的读取量不随章数增长。context_load 本身不通读其他历史记录文件。
...
```json
{
  "results": {
    ...
    "chapter_plan": {},
    "characters": [],
    "pending_items": [{"record_id": "第00000051_源00000050_0001", "path": "追踪/待处理事项/第00000051_源00000050_0001.md", "item": "..."}],
