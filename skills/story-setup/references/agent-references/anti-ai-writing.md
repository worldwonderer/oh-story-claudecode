...
| 系统公告公文腔过密 | 方括号规则/面板/公告行里硬规则词成片（`system-notice-formality-tic`）-> 保留规则功能，但把部分提示改成白话、具体后果或角色当下能理解的说法，避免像 API 文档 |
...
改法优先级：先删或原位替换污染句，不在段尾另补“人味”尾巴。需要补信息时，把原来的总结/动机/评价句改成角色当下能碰到的问题、手续、回信、付款、门外动静等具体压力；具体载体跟剧情走，不套固定清单。
...
| 叙述残留公文/文言腔（不得/须/未/已然/当前） | 换白话（不能/要/还没/现在）。系统公告、规则条文、面板播报可以保留冷硬功能；若 `system-notice-formality-tic` 报警，只白话化一部分，不删除规则功能 |
...
Zhuque CLI 可参考 <https://github.com/Sophomoresty/zhuque>。用户贴结果时，只描述检测结果，不要写“朱雀 0% AI / 100% 真人”或“固定公式”。

检测器只作旁证，不高于读感规则。对照样本里若出现“仿佛/非常/感到”等套词或告知式心理，仍按模式 1-8 清理；不要为了追检测结果机械补词、故意错字、按题材套壳或逐段追 conf。
...
3. 只在断裂处恢复自然连接和结构虚词；不设比例目标，不为外部检测注水。
4. 系统公告、规则条文、面板播报可以保留冷硬短句；`system-notice-formality-tic` 报警时，只白话化一部分硬规则词或补具体后果。
...
3. 合并后再删套词和告知心理：读顺不是恢复 AI 腔，不能为了检测结果把“仿佛/感到/非常/好像”成片加回来。

反证处理：即使清掉 `overcompressed-prose-tic` / `low-connective-density-tic`，外部检测仍可能不变。此时停止围绕单项指标微调，转为段落级重写或人工读感对照。
...
- [ ] **无电报体**：非峰值叙述保留自然虚词，「了下/了一下」式微动作不复读（micro-action-tic 不报警），删强调副词未伤语义
- [ ] **无抽象总结腔**：命运/棋局/这一刻终于明白/才刚刚开始不成串复现（abstract-summary-tic 不报警），结尾用动作、物件或未解决问题收束
- [ ] **无套词堆叠**：仿佛/一丝/一抹/深吸一口气/平静无波等高危套词不聚集（cliche-density-tic 不报警），修法是具体化，不是同义词轮换
- [ ] **无解释链报告腔**：知道/明白/这意味着/必须/需要等判断链不聚集（reasoning-chain-tic 不报警），修法是落到动作、物件、对话和现场反馈
- [ ] **无系统公告公文腔**：方括号规则/面板/公告行里的硬规则词不成片聚集（system-notice-formality-tic 不报警）；修法是保留规则功能，改成白话、具体后果或角色当下能理解的说法
- [ ] **无未解释的过度精炼短段/低连接密度**：长文本短叙述段不呈现提纲/电报体；若 overcompressed-prose-tic 只是有意短镜头且上下文读着顺，可记录为可留。叙述层自然连接不被扫光，且低功能词文本有足够中长承接句（low-connective-density-tic 不报警）；修法是通读后补断裂处、必要结构虚词和中长句群，禁止按指标注水
