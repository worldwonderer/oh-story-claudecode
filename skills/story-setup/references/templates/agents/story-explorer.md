...
1. `Grep "{角色名}" 追踪/角色状态.md` -> 取 `## 逐章事件流` 里该角色**章节号最大**的行，这是角色当前态的权威来源；快照块（文件上方按角色分的小节）只在事件流无命中时作参考——它可能停留在大纲期
2. `Glob 设定/角色/{name}*.md` -> `Read` 角色设定文件（基础人设：出身/性格/口头禅；**不含剧情变化**，当前态以第 1 步为准）
3. `Grep 正文/ "{角色名}"` -> 找到所有出场章节，`Read` 最近 1-2 章出场正文的相关段落（用行号定位）
4. 汇总返回：当前态 = 事件流最新行 + 最近出场正文互证；两者矛盾时以正文为准并在返回里标注
...
1. 指定了 ID 或关键词 -> `Grep 追踪/伏笔.md` 只取命中行；同一 ID 有多行时（含 `## 逐章事件流` 的追加行），当前状态以**最后一条命中**为准。要全量清单（foreshadow_list）才 `Read` 整个文件——文件可能长到上百 KB，读取留在本 agent 上下文内，返回只给筛选后的条目
...
1. 给了章节范围 -> `Grep "第.*章" 追踪/时间线.md` 取范围内的行（含 `## 逐章事件流` 的追加行）；要全书时间线才 `Read` 整个文件，读取留在本 agent 上下文内
2. 按章节范围筛选
3. 如需更多细节 -> `Read` 对应正文
4. 返回时间节点列表
...
1. `Read 追踪/上下文.md` -> 写作简报，日更所需状态（在场角色 / 活跃伏笔 / 近三章速记 / 长期约束 / 下一章硬任务）全在这里。如不存在，`Glob 正文/第*.md` 扫描最大章节号推断下一章编号
2. `Read 大纲/细纲_第{N}章.md` -> 本章写作计划
3. 从细纲提取角色名 -> `Read 设定/角色/{name}.md`
4. `Read 正文/第{N-1}章_*.md` -> 最新一章（衔接用）
5. **仅当**调用方在查询参数里明确给出伏笔 ID 或角色名，且简报里查不到时，才 `Grep` 对应冷文件（`追踪/伏笔.md` / `追踪/角色状态.md` / `追踪/时间线.md` / `追踪/章记/`）并只取最后 3 条命中。默认不读这些文件，它们已长到上百 KB
6. 汇总为"写作上下文包"

> 冷文件全文读取只在 `foreshadow_status` / `character_status` / `timeline` 这三个按需查询 query_type 里发生，且读取停留在本 agent 的上下文内、只把结构化摘要返给调用方——这样主会话的读取量不随章数增长。context_load 本身不做全量读取。
