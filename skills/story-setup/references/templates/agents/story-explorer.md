...
1. `Grep "^## {角色名}$" 追踪/角色状态.md` 定位文件上方该角色的状态小节，再从这个标题行 `Read` 到下一个 `## ` 标题（或文件末尾），取得身份、能力、关系和公众形象的基准值；另用 `Grep "{角色名}" 追踪/角色状态.md` 取得 `## 逐章更新记录` 中该角色的所有匹配行。不能只看带角色名的标题行，否则会漏掉小节内的基准值
2. **按维度分别归并**：将记录按身份、能力、关系、形象（含“公众形象”）分组，每个维度取章节号最大的记录，同章有多条时取文件中最后一条；**关系记录还要按关系对象分别归并**，例如“与沈栀”和“与林深”分别取各自最新值，不得互相覆盖。某维度或某个关系对象没有更新记录时保留状态小节中的基准值，两处都没有才标为未知。**不得把某一条单维度变更当成角色的完整当前状态**
...
2. 如果状态摘要 `## 历史记录索引` 标明有溢出待处理项，或 `Glob 追踪/逐章记录/_待处理事项.md` 命中，`Read` 该文件，把目标章节最早的项合并进 `下一章必做事项`；这是 context_load 唯一允许完整读取的额外追踪文件
...
> 除第 2 步中的专用待处理事项文件外，历史记录文件全文读取只在 `foreshadow_status` / `character_status` / `timeline` 这三个按需查询 query_type 里发生，且读取停留在本 agent 的上下文内、只把按固定字段整理的摘要返给调用方——这样主会话的读取量不随章数增长。context_load 本身不通读其他历史记录文件。
