...
原文明说的起因和理由照写（"存款花光了，他去找许新年借钱"里的起因是原文给的），只是不替角色推测原文没写出来的动机。
...
>   ],
>   "plot_points": [
>     {"id": "P<integer>", "event": "<白描：谁做了什么/结果如何；原文给出的起因一并写入>",
...
P{序号} **{事件概括}**：类型{转折点/信息揭示/冲突/解决/铺垫/行动/对话/状态变化} | {白描一句话：谁做了什么、结果如何；原文给出起因或理由的一并写进来，不推测动机；埋伏笔的写出伏笔线索} | 涉及{全名，多人逗号分隔；纯环境铺垫无具体人物时保留"涉及"标签、值留空} | 地点{如明确} | 物品{如涉及} | 时间{如明确}
...
2. 每个情节点是否使用了客观白描（无叙事框架词、无主观评价），且白描写全了事实、结果与原文给出的起因
3. 情节点是否严格按时序排列
