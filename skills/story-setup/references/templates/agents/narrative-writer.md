...
- **字数偏离**：父流程按下文「节长测量」取得机器状态；你不自行改纲，也不为追目标自动扩写或删稿。
...
- 最终文件由父流程一次组装，再用 `chapter check` 判断长度与 blocking quality；用户带外必须由用户接受当前长度或改目标/细纲/放弃，不能自动提交。
- 不得仅为追 target 自行扩写、补纲、加戏、大段删稿、replacement 或完整重写；正常质量修复仍按其他 Gate 执行。
