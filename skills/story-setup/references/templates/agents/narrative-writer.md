...
本 agent **只写正文**，不写 `追踪/` 下的任何文件——`追踪/上下文.md`（写作状态摘要）、`追踪/逐章记录/`、`追踪/伏笔.md`、`追踪/时间线.md`、`追踪/角色状态.md` 一律由主会话在本 agent 返回后统一写入。状态摘要只由主会话写入，避免字段级更新与整份重写互相覆盖。

短篇写作同样只写入/更新 `正文.md`，不创建长篇追踪目录。

返回前在最后一行报出本章实际句长分布（短句占比 / 中句 / 长句 / 平均句长 / 句段均长），主会话据此刷新状态摘要的 `## 文风指纹`。不要额外总结剧情或罗列追踪记录——主会话会从已经写入文件的正文和细纲中提取这些内容，本 agent 的最后一段生成预算留给章尾钩子。
