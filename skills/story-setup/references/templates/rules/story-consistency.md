...
4. **设定变更必须记录并分流**：重要设定决策一律记进当前章 `追踪/章记/第NNN章.md` 的「决策」行，并标 `[长期]` 或 `[一次性]`。其中对后续所有章永久生效的，同时写进 `追踪/上下文.md` 的 `## 长期约束`（≤6 行，pinned，只有用户显式撤销才能删）；只影响后续 3 章内的，写进 `## 下一章硬任务` 并标 `[pin]`（标了 `[pin]` 的行不参与淘汰，结清后删）。不新建 `## 最近决策` 区块：简报的顶层区块是固定白名单，写作流程会整份重写它，新建的区块下一章就被覆盖掉。
...
2. 记进 追踪/章记/第045章.md：决策：[长期] 沈栀灵力等级改为第六层；因影响后续所有章的战力判定，同步写进 追踪/上下文.md 的 ## 长期约束
