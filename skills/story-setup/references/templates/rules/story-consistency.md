...
4. **设定变更必须记录并按用途保存**：重要设定决策一律记进当前章 `追踪/逐章记录/第NNN章.md` 的「决策」行，并标 `[长期]` 或 `[一次性]`。其中对后续所有章永久生效的，同时写进 `追踪/上下文.md` 的 `## 长期约束`（≤6 行，固定保留，只有用户明确撤销才能删；第 7 条按写作流程停止并请用户指定合并方式）。只影响后续 3 章内、但需要跨章处理的事项，先用 `select-pending-items.py --next-name --target N --source M` 取得路径并写入单项文件，重新读取确认后，再按 `[固定｜目标第N章｜来源第M章｜记录编号={record_id}] 事项` 写进 `## 下一章必做事项`。所有未结清跨章事项都保留单项文件，摘要只显示脚本选出的最早事项；不新建 `## 最近决策` 区块。
