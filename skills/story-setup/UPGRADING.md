...
- **追踪文件不再无界膨胀**：`追踪/上下文.md` 从「进度快照」改为**写作简报**——顶层区块固定 11 个（当前位置 / 文风指纹 / 长期约束 / 在场角色 / 活跃伏笔 / 近三章速记 / 下一章硬任务 / 累计欠账 / 十章概要 / 待办 / 冷文件索引），各带条数上限，硬上限 12288 字节，每章由主会话整份重写。日更每章的必读来源从五个文件收缩到三项（本章细纲 + 上一章正文 + 简报）；`追踪/伏笔.md`、`追踪/时间线.md`、`追踪/角色状态.md` 降为冷文件，日更不读，需要旧信息时走 story-explorer 按需查询或 grep。全书历史改存 `追踪/章记/第NNN章.md`（每章一个小文件，日更不读），长周期概要存 `追踪/卷史.md`。此前写到 30 章时这四个文件合计可达 400KB 以上、且每章全读一遍。
- **`上下文.md.tmpl` 换成简报模板**：删去 `## 最近决策`、`## 待处理线索`、`## 本次写作变更`、`## 已写内容分层摘要`（含「30 章后启用分层摘要」与十章概要 / 卷级总览子块）；新增 `## 长期约束`（pinned，只有用户显式撤销才能删）、`## 在场角色`、`## 活跃伏笔`（带重要度，高重要度至少保留 2 条）、`## 近三章速记`、`## 下一章硬任务`、`## 累计欠账`、`## 十章概要`（滚动 4 行）、`## 冷文件索引`。该文件是 create-only-if-absent，只影响新项目。
- **简报只有一个写入方**：`narrative-writer` 不再写 `追踪/` 下任何文件（原「完成后自动更新 上下文.md」整节删除），只写正文并在返回末行报出本章句长分布；追踪落盘统一由主会话在 agent 返回后执行。此前主会话按块追加、agent 按字段原地更新，两套语义的产物在同一文件里并存。
- **`story-explorer` 的 `context_load` 不再全读冷文件**：只读简报 + 细纲 + 角色设定 + 上一章正文；仅当调用方明确给出伏笔 ID 或角色名且简报查不到时才 grep 冷文件并取最后 3 条命中。冷文件全文读取只发生在 `foreshadow_status` / `character_status` / `timeline` 三个按需查询里，且停留在该 agent 的上下文内。
- **`story-consistency.md` 规则 4 改为分流**：设定决策一律记进当前章 `追踪/章记/第NNN章.md` 并标 `[长期]` / `[一次性]`；永久生效的同时进简报 `## 长期约束`，只影响后续 3 章的进 `## 下一章硬任务`，不再往无上限的 `## 最近决策` 堆。
- **会话起点摘要行数调整**：`session-start.sh` 的上下文摘要从前 10 行改为前 18 行，覆盖新模板的 `## 当前位置` 整块（8 字段）。
- **会话起点新增简报体积告警**：`追踪/上下文.md` 超过 12288 字节时报出当前体积与处置方式（搬进 `追踪/章记/` 后整份重写，不是删）。四端同步：`story_hook_core.js`（Claude Code / ZCode / OpenCode 三份字节一致）与 Codex 的 `story_codex_hook.py`。
- **`/story-review` 新增追踪文件维护**：过期伏笔标记（`已过期` 的唯一写入方）、角色快照重建、简报核对、冷文件体积收敛。日更只追加不清理，清理统一归审查。
- **story-explorer 查询对齐事件流**：`character_status` 优先读 `追踪/角色状态.md` 逐章事件流的最新行（不再拿 Phase 3 原始人设当当前态）；`foreshadow_status` / `timeline` 按需 grep 命中行，全量读取仅在明确要全量清单时。
