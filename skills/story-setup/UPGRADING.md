...
- 所有可能 spawn 项目 agent 的 Skill 都先读取 `.story-deployed.agents_version`：仅 v23 才继续检查 agent 文件；标记缺失、非整数、旧于 v23 或高于 v23 时均不得 spawn，降级 solo/direct。磁盘上残留 agent 文件不能替代版本校验。
...
5. 重新运行 `/story-setup`（Codex 用 `$story-setup`）并新开会话，使 v23 的 agent 模板和 spawn 门禁生效。
...
- **统一 agent 版本门禁（F-011）**：所有 spawn-capable Skill 都以 `.story-deployed.agents_version` 为唯一可用性版本权威；仅当前 v23 才继续文件/运行时检查，旧版残留 agent 文件不再被误用。
