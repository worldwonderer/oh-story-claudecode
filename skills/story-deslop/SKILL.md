...
### 边界：去AI味 ≠ 过第三方 AIGC 检测器

去AI味治的是读感——模板腔、书面化、过度工整。第三方 AIGC 检测器（如朱雀）测的是 token 统计分布，与本 skill 的指纹清单正交：过了这里全部 Gate，仍可能被判高疑似；反过来靠注水、打乱句式、故意错标点去压检测分，会连带压掉爽点和可读性。本 skill 不以检测器通过率为目标，也不承诺能过检；用户若另有过检需求，需自行处理，不要把去AI味当过检工具。

若用户提供朱雀结果，报告措辞必须按字段说清：`labels_ratio.1=0` 只能说“当前未判 AI”，不能写成“朱雀 0% AI / 100% 真人”；分段 `label=2` 是“不确定”，不算过检成功。只有 `label=0` 或 `labels_ratio.0` 占优，才可描述为“朱雀当前判真人/偏真人”。本地复扫 0 findings 但朱雀仍 `label=2` 时，按 `anti-ai-writing.md` 模式 9 的“顺读补全”处理，禁止为了追 `label=0` 全局注水结构虚词。
...
- 将 `not-is-comparison` 结果并入 Gate B「句式套路」；脚本会覆盖同句、逗号/分号/冒号后、紧凑写法以及相邻行肯定翻转。`em-dash`（破折号按功能改写）也并入 Gate B；`period-stutter`（碎句号）、`long-paragraph`（长段落）、`micro-action-tic`（「了下/了一下」式微动作复读，电报体指纹）、`abstract-summary-tic`（命运/棋局/这一刻终于明白/才刚刚开始，AI 结尾腔）、`cliche-density-tic`（仿佛/一丝/深吸一口气/平静无波等套词密度过高）作为节奏/解释腔提示。
...
作用边界：
- `check-ai-patterns.js` 报告七类需按语义/功能改写的问题：① 高危 AI 对比句式；② 破折号（按功能改写：打断→动作/短句，拖长音→省略或动作，插入说明→逗号/冒号，**不要一律改句号**）；③ 碎句号（连续短叙述句无呼吸→按目标句长合并成中长句）；④ 长段落（>200 字→按镜头/动作/视线断段）；⑤ 微动作复读（「了下/了一下」式轻反应高密度→按 anti-ai-writing.md 模式 9 合并动作、换具体细节）；⑥ 抽象总结复读（命运/棋局/这一刻终于明白/才刚刚开始→按 anti-ai-writing.md 模式 8 回到文件、动作、对话或物理后果）；⑦ 套词密度过高（仿佛/一丝/深吸一口气/平静无波等成串→回到角色当下证据，禁止同义词轮换）。①② 必须回 Gate B 改掉、复扫到 0；③④⑤⑥⑦ 是提示，确属问题就改、justified 长推理段可留。只报告不改写。
