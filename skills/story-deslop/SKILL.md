...
- 删除比例上限：单次去AI味操作的删除量不得超过原文总字数的 15%。超过此比例应提醒用户并建议分段处理
- 对于"疑似AI味但不确定"的内容，保留并标注 `[建议人工审核]`，而非直接删除
...
如果该文件存在，其中的词汇不会被 Gate A 标记为禁用词。
...
如果 `.deslop-whitelist` 不存在，首次运行时自动创建并提示用户可自定义。
