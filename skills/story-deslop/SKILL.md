...
同一段文本被重复要求“再去一遍味/再润一下/继续优化”时，默认视为**覆盖当前终版**，不是另存新版本。若用户明确要求保留多个版本，再单独输出草稿A/B。
...
- 如果用户提供的是某个文件路径，默认**原地覆盖**该文件对应的终版文本。
- 如果用户只贴文本，不指定文件，默认只输出润色后的终版，不生成额外草稿。
- 反复润色同一文本时，先以前一版终版为基准，不回滚到废稿或草稿。
...
| 用户对同一段/同一章反复润色 | 默认覆盖当前终版，不创建新版本 |
...
润色完成后先输出润色结果，再自动补回对应的固定菜单；不自动执行菜单中的下一步：
- 长篇 → `story-long-write`
- 短篇 → `story-short-write`
- 如需复审，先返回 `story-review`
