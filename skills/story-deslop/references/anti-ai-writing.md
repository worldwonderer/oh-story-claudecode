...
| 叙述残留公文/文言腔（不得/须/未/已然/当前） | 换白话（不能/要/还没/现在）。例外：系统公告、规则条文、面板播报等作品内文本保留冷硬电报体，那是设定的一部分 |
...
`labels_ratio.1 = 0` 只表示朱雀当前未判 AI；若分段 `label = 2`，含义是不确定，不等于真人。issue #205 真实附件复测中，原始章本地只剩少量句式问题时可降到 `label 2`，人工过检版是 `label 0`，但人工版的主要差异之一是结构虚词密度上升；不要把它误读成“全局补的/了/就即可过检”。

当 `check-ai-patterns.js` 已复扫到 0 blocking / 0 明显 advisory，而第三方检测仍是 `label 2` 或读起来仍像提纲时，只做一轮顺读补全：

1. 标出读起来像逻辑报告的段落：连续出现“他知道/他明白/这意味着/真正的问题/在这种情况下/必须/需要”等判断链，却缺少当下动作、物件或对话反馈。
2. 把叙述者结论落地：用角色手里的清单、门牌、广播键、纸箱、脚步、旁人反应等具体证据替代“他意识到/他清楚/这意味着”。
3. 只在断裂处恢复自然连接和结构虚词：非峰值叙述允许“了/的/就/的时候/已经”等中文冗余；不设比例目标，不为追求 `label 0` 注水。
4. 作品内系统公告、规则条文、面板播报可以保留冷硬短句；那是设定文本，不按日常叙述回填。
