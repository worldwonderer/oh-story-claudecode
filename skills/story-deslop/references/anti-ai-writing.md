...
本文件以问题模式和高危清单为主。一级高危词优先检查；二级/语境敏感词按频率、语境和是否偷懒判断。遇到冲突时，保留作者意图与剧情功能优先于机械替换。
...
**检测模式**：一个动作或情绪先写发生，再补感知细节，再补身体反应，分成多段依次写完。读者看到的是同一个瞬间被拆开解释三遍，而不是一个连续画面。

**典型特征**：
- 先写概括动作，再另起段补细节，再另起段补身体反应
- “发生→感知→反应”按顺序分段出现，而不是织入同一段正文
- 段落看似细腻，但每段都在围绕同一动作或同一情绪打转
...
> 手在抖。
>
> 不是小抖，是从肘到腕的那种，笔尖在纸上停了停，写了一横，又停。

→ 同一个“写字时手抖”的瞬间被拆成三段，AI 味会变重。

**正确做法**：把发生、感知、反应三个维度织入同一段连续正文：
