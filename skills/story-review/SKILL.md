...
2. **范围传递策略**：
   ...
   - **跨批连续性（分批必做）**：审每一批前，先读 `追踪/伏笔.md` 里「已埋未回收 / 未埋」且预计回收章 ≤ 本批末章的开放项（同一 ID 有多行时以最后一条记录为当前状态，含 `## 逐章事件流` 的追加行），连同上一批 findings 摘要，作为「继承的开放项」注入本批 reviewer / consistency-checker prompt（与既有「已知角色」并列）——这样审 200-300 时能看见 1-200 埋下、本批本该兑现却悬空的钩子/伏笔/未完成剧情，跨批不断线。审完把本批新发现、且不在 `伏笔.md` 里的开放钩子补登记进 `追踪/伏笔.md`（续写/import 工程常见 reviewer 先于写手发现）。
...
长篇项目的 `追踪/` 冷文件按只追加方式维护，日更不做清理；清理与重算是本 skill 的职责。full / lean 模式审查收尾时执行，solo 模式提示用户可执行。审查路径允许全量读取冷文件。

1. **过期标记**：读 `追踪/伏笔.md`，对当前状态为 `未埋`/`已埋`（同一 ID 以最后一条记录为准）且预计回收章 < 当前章的条目，往文件末尾 `## 逐章事件流` 追加一行，六列序与原表一致、状态写 `已过期`。日更流程不写 `已过期`，本步是它唯一的写入方——不执行，SessionStart 的过期伏笔提醒就永远不会触发。
2. **角色快照重建**：按 `追踪/角色状态.md` 的 `## 逐章事件流` 重算主要角色当前态，重写文件上方各角色快照块；重算后可删除已并入快照的旧事件行。这是唯一允许对该文件整体重写的地方。
3. **简报核对**：`追踪/上下文.md` 顶层区块符合简报契约（11 个白名单区块、条数上限、≤12288 字节）；`## 活跃伏笔` 与伏笔文件当前状态一致——漏掉的高重要度开放伏笔补进简报或记入 findings。
4. **冷文件体积**：单个冷文件超过 256KB 时，顺带做一次第 2 步式的快照重建加旧事件行清理，把当前态收敛进快照块。
