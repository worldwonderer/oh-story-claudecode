...
长篇项目的 `追踪/` 历史记录文件按只追加方式维护，日更不做清理；清理与重算是本 skill 的职责。**full / lean 模式只允许修改 `追踪/` 下的文件**，审查收尾时按下列步骤执行；**solo 模式只报告维护建议，不修改任何文件**。三种模式都可完整读取历史记录文件；除本节明确列出的 `追踪/` 维护外，不得写入其他文件。
...
2. **开放钩子补登记**：对 Phase 1 各批列出的维护候选去重。full / lean 模式把有明确正文证据、且 `追踪/伏笔.md` 中确实没有的开放钩子追加到 `## 逐章更新记录`；证据不足的只写进 findings。solo 模式两种都只报告，不写文件。
3. **重算角色当前状态**：按 `追踪/角色状态.md` 的更新规则，以上方角色状态小节为基准，合并 `## 逐章更新记录`；身份、能力、公众形象按维度分别取最新值，关系再按关系对象分开取最新值。**重算时不得只取该角色的最后一条记录**。完成后重写文件上方各角色状态小节；重算后可删除已整理到角色状态小节的旧记录。这是唯一允许对该文件整体重写的地方。
4. **状态摘要核对**：`追踪/上下文.md` 顶层区块符合状态摘要格式（11 个规定区块、条数上限、≤12288 字节）；`## 活跃伏笔` 与伏笔文件当前状态一致——漏掉的高重要度开放伏笔按写作流程的排序和 8 条上限补进状态摘要，其余保留在伏笔文件并记入 findings。
5. **历史记录文件体积**：`追踪/角色状态.md` 超过 256KB 时执行第 3 步，删除已整理到角色状态小节的旧记录。其他历史记录文件超过 256KB 时只在 findings 中报告；没有对应的重建规则时不得套用角色状态的清理方式或自行删除记录。
