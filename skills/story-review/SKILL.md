...
2. **范围传递策略**：
   ...
   - **跨批连续性（分批必做）**：审每一批前，先读 `追踪/伏笔.md` 里「已埋未回收 / 未埋」且预计回收章 ≤ 本批末章的开放项（同一 ID 有多行时以最后一条记录为当前状态，含 `## 逐章更新记录` 的追加行），连同上一批 findings 摘要，作为「继承的开放项」注入本批 reviewer / consistency-checker prompt（与既有「已知角色」并列）——这样审 200-300 时能看见 1-200 埋下、本批本该兑现却悬空的钩子/伏笔/未完成剧情，跨批不断线。本批新发现、且不在 `伏笔.md` 里的开放钩子只先列入本批维护候选；是否写入由收尾阶段的「追踪文件维护」统一决定。
...
6. **确定性预检（只报告，不修改）**：当审查范围包含本地正文文件路径时，运行本 skill 自带脚本：
   ...
   - 这三个预检脚本只读；`story-review` **不修改正文、设定或大纲文件**，需要自动修复正文时建议转 `/story-deslop`。三种模式只有下方「追踪文件维护」允许写文件，模式差异只决定是否调用 reviewer agents，不改变追踪维护结果。
...
长篇项目的 `追踪/` 历史记录文件按只追加方式维护，日更不做清理；清理与重算是本 skill 的职责。full、lean、solo **三种模式都执行同一套确定性的追踪维护**，且**只允许修改 `追踪/` 下的文件**；模式差异只在审查代理数量。三种模式都可完整读取历史记录文件；除本节明确列出的 `追踪/` 维护外，不得写入其他文件。当前主会话没有文件写权限时，报告“追踪维护未执行”及具体待处理项，不得声称已经维护。

第 1、2 步的追加必须复用 workflow-daily 的可靠写入规则：`## 逐章更新记录` 不存在或不在末尾时，先在文件末尾补标题和正确表头；每次追加后用 `tail` 与精确固定字符串核对，失败时重试一次。重试仍失败时，在状态摘要“质量预警”和审查报告写明缺失行，停止后续维护，不得声称维护完成。

1. **过期标记**：读 `追踪/伏笔.md`，对当前状态为 `未埋`/`已埋`（同一 ID 以最后一条记录为准）且预计回收章 < 当前章的条目，往文件末尾 `## 逐章更新记录` 追加一行，六列序与原表一致、状态写 `已过期`。日更流程不写 `已过期`，本步是它唯一的写入方——不执行，SessionStart 的过期伏笔提醒就永远不会触发。
2. **开放钩子补登记**：对 Phase 1 各批列出的维护候选去重。所有模式都把有明确正文证据、且 `追踪/伏笔.md` 中确实没有的开放钩子追加到 `## 逐章更新记录`；证据不足的只写进 findings，不补登记。
3. **重算角色当前状态**：按 `追踪/角色状态.md` 的更新规则，以上方角色状态小节为基准，只合并角色列精确匹配的 `## 逐章更新记录`；身份、能力、公众形象按维度分别取最新值，关系从 `与{对象}：{当前关系}` 中解析对象，再按关系对象分开取最新值。旧记录无法确定关系对象时，只有正文或状态小节能唯一确定对象才合并，否则列入 findings 待人工确认。**重算时不得只取该角色的最后一条记录**。完成后重写文件上方各角色状态小节。需要整理旧记录时，先确保 `追踪/角色状态归档/` 已创建，只归档并移除已经成功合并的行；**未成功合并的行必须留在当前 `## 逐章更新记录` 中，直到问题解决**。归档名使用 `第{起始章补足三位}-{截止章补足三位}_{YYYYMMDD-HHMMSS-ffffff}.md`；若仍与既有文件重名，在末尾追加 `_01`、`_02` 直到得到新路径，绝不覆盖。将移出的行完整写入并核对后，再从当前文件移除；必须**先归档，再从当前文件移除**，不得直接删除。这是唯一允许对该文件整体重写的地方。
4. **状态摘要核对**：`追踪/上下文.md` 顶层区块符合状态摘要格式（11 个规定区块、条数上限、≤12288 字节）。第 3 步重算后，保持 `## 在场角色` 现有的入选顺序和最多 6 人上限，用重算结果刷新其中每个角色的身份与当前状态；角色不在该区块时不擅自挤入。`## 活跃伏笔` 与伏笔文件当前状态一致——漏掉的高重要度开放伏笔按写作流程的排序和 8 条上限补进状态摘要，其余保留在伏笔文件并记入 findings。
5. **历史记录文件体积**：`追踪/角色状态.md` 超过 256KB 时执行第 3 步的归档与整理；确认归档文件已经写入、行数与待移出行一致后，才移出当前文件里的旧记录。其他历史记录文件超过 256KB 时只在 findings 中报告；没有对应的归档/重建规则时不得套用角色状态的整理方式或自行删除记录。
