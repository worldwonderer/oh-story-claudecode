...
> **句长总纲**：规则 3 是全文句长的唯一基线。规则 1-4 及本文件一切「短句/拆短/能删就删」表述，与规则 3 冲突时服从规则 3。
...
叙述（旁白）的底色是**逗号流水句**：一句用逗号串起 2-4 个动作/信息点再落句号，逗号间片段 8-12 字、整句 20-30 字。短句不是叙述的默认形态，只在段落层分节拍成簇使用，簇后回到流水句。

| 场景 | 句长 | 示例（长篇语料原句） |
|------|------|------|
| 叙述底色（日常/推进/描写） | 逗号间 8-12 字，整句 20-30 字 | 阴冷潮湿的气息扑面而来，身下铺着一层薄薄的稻草，湿漉漉地粘在皮肤上。 |
| 打斗/冲突/情绪顶点 | 短句簇、逐句成段，向顶点压短 | 可他现在什么也做不了。他只能努力活着。活下去。 |
| 悬念钩子/冲击揭示 | 单句短段 | 虽然过程并不美好。 |
| 对话 | 口语化，长短随角色 | "你疯了？""可能吧。" |

短句簇只用于三种节拍：悬念钩子、冲击揭示、情绪顶点；真实语料中簇以逐句成段出现（一句一段连排），不是段内句号连打；簇后立刻回流水句。

**红线（与 AI 腔同级的不合格）**：
- 逗号间碎片 ≤5 字连续成串（"他抬手，开门，进屋，坐下"式）
- 通篇 3-8 字句、句号密集如提纲（电报体，见模式 9）
- 机械长短交替（一长一短循环同样是模板）

> **爆款语料校准**（七猫长篇 现言/都市/古言/玄幻/历史 125 本×前 8 章旁白统计）：逗号间片段均值 8.8-9.6 字；整句均值 22-24 字；逗号流水句占叙述句 74-80%；≤5 字片段约占两成，多为孤立的时间词、转折、动作重拍。短篇（盐言体）段落更碎（≤15 字单句段可近半，长篇约两三成），但逗号层节奏与长篇一致：**段落随体裁变短，逗号层不碎**。
...
> 以下范例处理的是臃肿修饰、堆叠比喻和抽象总结，不是「见长就拆」：改写后叙述主体仍是逗号流水句（规则 3），不要把正常流水句拆成短句串。
...
- [ ] **句长基线达标**：叙述底色是逗号流水句（逗号间 8-12 字、整句 20-30 字，规则 3）；打斗/冲突/情绪顶点短句成簇，簇后回流水句；无 ≤5 字碎片连续成串，无通篇短句提纲
