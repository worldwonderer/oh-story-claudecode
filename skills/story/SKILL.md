...
以下规则适用于所有写作类 skill（包括但不限于 `story-long-write`、`story-short-write`）：

1. 每个“章节/小节/场景批次”结束后，skill 必须只输出其定义的固定收尾提示，不得附加总结、建议、解释、预判或其他内容。
2. 当用户重复要求重写同一章节/小节时，默认覆盖当前终版，不自动生成多版本草稿，除非用户明确要求保留版本。
3. 润色、审查完成后可以自动回显对应的固定菜单，但不得自动替用户执行菜单中的下一步。
