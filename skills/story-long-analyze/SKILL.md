...
| 3 | 聚合分析 | 全部章节摘要 | 剧情/*.md + 故事线.md。**故事框架识别**（前置，决定聚合策略）。**两步法剧情聚合**（先从摘要识别剧情大纲，再按大纲分配情节点）。**角色合并**（跨章节去重+别名归一）。**角色分级**（主角/核心配角/功能角色/路人）。**孤立情节兜底**（6步+覆盖率验证）。**质量门控**（置信度≥0.85/覆盖率85%-95%/重叠率≤35%）。 | 质量检查通过 |
