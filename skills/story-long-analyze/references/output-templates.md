...
每章8-15个（按字数动态调节：3000字→8-10个，5000字→10-12个，8000+字→12-15个）。太粗（整段打架）→丢细节；太细（举起右手）→碎片化。恰当：完整戏剧事件（"主角三招击败反派，围观者震惊"）。
...
`角色/角色关系.md`（每5章批量）：A<->B：关系类型 | 情感 | 描述（50-200字）| 演变轨迹
