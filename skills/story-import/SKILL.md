...
> 此文件不可遗漏。它是 `追踪/上下文.md` 的 `## 在场角色` 的数据来源，也是后续 `character_status` 按需查询的检索源；缺失会让续写从第一章起就拿不到角色当前态。日更本身不读它（冷文件）。

**④ 追踪/上下文.md**：**写作简报**，最后生成（`## 在场角色` 从 `角色状态.md` 取，`## 活跃伏笔` 从 `伏笔.md` 取）。顶层区块必须恰好是下面 11 个、不多不少——续写流程按这个白名单校验，多出或缺失的区块会在首次日更时被整份重写：
...
## 十章概要
- {每 10 章一行：第_-_章 | 核心事件 | 角色状态变化；只留最近 4 行，全量写 追踪/卷史.md}
...
- [ ] 上下文.md 的顶层区块恰好是简报契约那 11 个（多出或缺失会在首次日更被整份重写）
- [ ] 追踪/章记/ 空目录已创建（续写起点；导入章无章记，回查走伏笔/角色状态事件流与正文）
- [ ] 追踪/卷史.md 已生成（十章概要全量 + 卷级总览，从章节摘要汇总）
- [ ] 卷划分已确认（原文无明确卷界时）
