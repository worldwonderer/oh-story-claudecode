...
**⑥ 追踪/上下文.md**：**写作状态摘要**，最后生成（`## 在场角色` 从 `角色状态.md` 取，`## 活跃伏笔` 从 `伏笔.md` 取，`## 十章概要` 从 `阶段摘要.md` 取最近 4 行）。顶层区块必须恰好是下面 11 个、不多不少——续写流程按这 11 个规定区块校验，多出或缺失的区块会在首次日更时被整份重写：
