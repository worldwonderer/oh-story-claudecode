...
追踪目录包含六项产物（五个文件和一个目录），**必须按以下顺序生成**（后一项可能依赖前面的产出）：

```
① 追踪/伏笔.md     ← 先生成（角色状态反推的「待回收伏笔」字段依赖此文件）
② 追踪/时间线.md   ← 与伏笔同步或紧随其后
③ 追踪/角色状态.md ← 依赖伏笔.md 已存在
④ 追踪/逐章记录/   ← 执行 `mkdir -p 追踪/逐章记录`；导入章不补造逐章记录
⑤ 追踪/阶段摘要.md ← 从章节摘要与卷划分汇总整本已导入内容
⑥ 追踪/上下文.md   ← 最后生成（其「在场角色」依赖角色状态.md，「十章概要」取阶段摘要最近 4 行）
```
...
## 逐章更新记录
<!-- 后续日更与审查只往这里追加；本小节必须保持为文件最后一个小节 -->
| ID | 伏笔内容 | 埋设章节 | 预计回收章节 | 状态 | 重要度 |
|----|---------|---------|-------------|------|--------|
```
...
> 此文件不可遗漏。它是 `追踪/上下文.md` 的 `## 在场角色` 的数据来源；后续 `character_status` 按需查询也以它为依据；缺失会让续写从第一章起就拿不到角色当前状态。日更本身不读它（历史记录文件）。

**④ 追踪/逐章记录/**：执行 `mkdir -p 追踪/逐章记录`。导入已有小说时不为旧章伪造日更过程记录，目录保持为空；续写的第一章起再按 workflow-daily 逐章写入。

**⑤ 追踪/阶段摘要.md**：从 `章节/第N章_摘要.md` 与已确认卷划分生成。按每 10 章一组汇总，最后不足 10 章的区间也要写入；每卷再写一行总览，确保报告中的“已生成”有对应文件。
...
**⑥ 追踪/上下文.md**：**写作状态摘要**，最后生成（`## 在场角色` 从 `角色状态.md` 取，`## 活跃伏笔` 从 `伏笔.md` 取，`## 十章概要` 从 `阶段摘要.md` 取最近 4 行）。顶层区块必须恰好是下面 11 个、不多不少——续写流程按这 11 个规定区块校验，多出或缺失的区块会在首次日更时被整份重写。状态摘要不得超过 12288 字节；`当前位置` ≤8 行、`文风指纹` ≤4 行、`长期约束` ≤6 行、`在场角色` ≤6 人、`活跃伏笔` ≤8 条、`## 近三章速记` 最多 3 条、`下一章必做事项` / `累计待处理项` / `待办` 各 ≤5 行、`十章概要` ≤4 行、`历史记录索引` ≤6 行。导入时就按这些上限筛选，不把超限内容留给首次日更处理：
...
- 卷: 第{X}卷·{卷名}
- 本卷主线: {一句话}
- 章: 第 {N} 章（导入 {N} 章，共 {X} 字）
- 故事时间: {最后一章对应的故事时间}
- 场景: {最后一章收束在哪}
- 情绪目标: {下一章目标}
- 承诺进度: {原书挂着的最大期待债}
- 质量预警: {最后一章为残稿时注明残稿到第 N 章及用户选定的续写策略}

## 文风指纹
<!-- 每次完整重写，≤4 行 -->
- 目标句长带: 短<15字 _% / 中15-30 _% / 长>30 _% / 平均 _字 / 句段均长 _字
- 来源: ___（设定/文风.md 自定义 / 对标 文风.md / 原文锚点）

## 长期约束
<!-- 固定保留，≤6 行；只有用户明确撤销才能删 -->
- {原书已确立、后续必须遵守的硬设定；无则留空}

## 在场角色
<!-- 每次完整重写；按下一章细纲出场顺序取人，再补主角和核心角色，去重后 ≤6 人，每人 1 行 ≤60 字 -->
- {按续写第 N+1 章细纲的出场顺序取人，再补主角和核心角色，去重后最多 6 人；格式：角色名｜身份｜当前状态（第N章变化）}

## 活跃伏笔
<!-- 每次完整重写，≤8 条；筛选顺序以 workflow-daily 为准，移出的条目仍留在 追踪/伏笔.md -->
- F0__｜{按日更规则筛到最多 8 条：先选最多 2 条最紧迫的高重要度项，再按逾期、重要度、计划回收章和 ID 补足；一句话}｜埋第_章｜计划第_章回收｜{高/中/低}

## 近三章速记
<!-- 每次完整重写，最多 3 条，每条 ≤80 字；不足 3 章只写实际存在的章节 -->
- {按实际存在的章节列出最近最多 3 条，每条 80 字内；不足 3 章时不补不存在的章节}

## 下一章必做事项
<!-- 每次完整重写，≤5 行；导入时只写下一章的一次性事项。续写后如产生跨章事项，再由日更流程创建单项文件和 record_id -->
- {续写第 N+1 章必须处理的一次性事项；没有则留空}

## 累计待处理项
<!-- 每次写入当前完整数值，不追加，≤5 行；计数下降必须有逐章结清记录 -->
- {无则留空；导入书从零起算}

## 十章概要
<!-- 只保留最近 4 行；完整记录在 追踪/阶段摘要.md -->
- {每 10 章一行：第_-_章 | 核心事件 | 角色状态变化；只留最近 4 行，完整内容写入 追踪/阶段摘要.md}
...
## 历史记录索引
<!-- 每次完整重写，≤6 行；待处理事项以目录选择脚本的结果为准 -->
- 追踪/伏笔.md _行｜全部伏笔记录
- 追踪/角色状态.md _行｜角色逐章变更记录
- 追踪/时间线.md _行｜逐章时序
- 追踪/逐章记录/ _个文件｜续写后每章一份完整记录；导入时为空
- 追踪/阶段摘要.md _行｜完整十章概要与卷级总览
<!-- 续写后有未结清跨章事项时才添加第 6 行：追踪/待处理事项/ _项｜最早第_章｜最早文件路径___ -->
```
...
- 追踪：伏笔.md + 时间线.md + 角色状态.md + 逐章记录/ + 阶段摘要.md + 上下文.md
- 设定：{世界观文件数} 个
...
- [ ] 题材定位的核心梗三分法需要确认
- [ ] 伏笔追踪中的伏笔已复核
- [ ] 角色状态.md 已复核（`## 在场角色` 与 character_status 按需查询的来源）
- [ ] 上下文.md 的顶层区块恰好是状态摘要格式规定的 11 个（多出或缺失会在首次日更被整份重写）
- [ ] 上下文.md 各区块未超规定条数，且状态摘要不得超过 12288 字节
- [ ] 追踪/逐章记录/ 空目录已创建（续写起点；导入章无逐章记录，需要查找旧信息时读取伏笔记录、角色状态记录或正文）
- [ ] 追踪/阶段摘要.md 已生成（完整十章概要 + 卷级总览，从章节摘要汇总）
- [ ] 卷划分已确认（原文无明确卷界时）
