...
│   └── 金手指.md
├── 拆文报告.md
├── 文风画像.md          # Stage 6 派生：写作技法视图 + 原文 few-shot 锚点
└── _progress.md
```
...
| 6 | 文风画像派生 | 拆文报告.md + 章节/第1-3章_深度拆解.md + 章节/*_摘要.md + 原文/原文.txt | 文风画像.md（整书级写作技法视图，story-long-write 日更循环必读） | 画像落盘 `拆文库/{书名}/文风画像.md` |
...
```yaml
主对标书: {书名}  # 多本对标时日更默认用哪本的文风画像；缺失时 story-long-write 用字典序第一本并提示用户补
对标书列表:
  - 书名: {书名 A}
    引用强度: 主  # 主 / 辅 / 参考
  - 书名: {书名 B}
...
把 `拆文库/{书名}/文风画像.md` 复制到 `{项目}/对标/{书名}/文风画像.md`。纯复制，不派生。

**缺失处理**：

- 拆文库没有画像（analyze 未跑 Stage 6）→ 导入报告提示用户重跑 `/story-long-analyze` 后再同步；日更前画像缺失会被 fail-fast 拦截
- 项目对标已有旧画像 → 覆盖（最新拆文产物优先），在导入报告告知
...
- [ ] 卷划分已确认（原文无明确卷界时）
- [ ] 文风画像已从拆文库同步到 `对标/{书名}/`（日更前 fail-fast 依赖此文件）
