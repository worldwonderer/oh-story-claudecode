...
>
> 生成顺序约束：本文件所描述的反推依赖 `追踪/伏笔.md` 已生成（步骤 6「待回收伏笔」需查伏笔状态）。Phase 3-L 的完整顺序是：**先生成 `伏笔.md` 和 `时间线.md` → 再生成 `角色状态.md`（本规则）→ 创建 `逐章记录/` 并生成 `阶段摘要.md` → 最后生成 `上下文.md`**（写作状态摘要的 `## 在场角色` 依赖本文件）。
...
## 逐章更新记录
<!-- 后续日更只往这里追加；本小节必须放在所有角色小节之后，并保持为文件最后一个小节 -->
| 章节 | 角色 | 维度{身份/能力/关系/形象} | 当前值 |
|------|------|------------------------|--------|
```

每个需追踪的角色独立一个 `## {角色名}` 节，按主角→反派→核心配角顺序排列；所有角色小节写完后再写唯一的 `## 逐章更新记录`，不能把它重复放进每个角色小节。
