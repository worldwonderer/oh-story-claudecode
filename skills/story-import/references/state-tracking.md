...
2. 如果有变化：往 `## 逐章更新记录` **追加**一行 `| 第N章 | 角色 | 维度 | 变化 |`，同时把该角色的当前状态写进写作状态摘要的 `## 在场角色`。日更不修改上方角色状态小节——那需要先读整个文件，而它会长到上百 KB
3. 如果无变化：跳过
4. 仅追踪主要角色（出场 ≥3 次或有独立剧情线的角色），不追踪路人角色
5. 重算角色当前状态：`/story-review` 或导入时以上方角色状态小节为基准，再合并 `## 逐章更新记录`。**身份、能力、公众形象分别取各自的最新记录**；**关系还要按关系对象分开重算**，每个对象取自己的最新记录；同章多条取文件中最后一条。不得用该角色的最后一条单维度记录覆盖其他状态。重算后可清掉已并入字段的旧记录。日更不做这件事
