...
`追踪/角色状态.md` 用于追踪主要角色的状态变化。Phase 3 大纲完成后创建初始状态。它是**冷文件**：日更每章不读它，只往末尾的 `## 逐章事件流` 追加变更行；上方的角色快照块由 `/story-review` 或导入时重建。日更所需的角色当前态取自写作简报 `追踪/上下文.md` 的 `## 在场角色`。
...
> 用途：角色变更的全量检索源，供 story-explorer 的 character_status 查询和 /story-review 使用。日更不读本文件。
> 更新时机：Phase 3 大纲完成后创建初始状态；日更每章只追加 `## 逐章事件流` 的行；快照块由 /story-review 或导入时重建。
...
2. 如果有变化：往 `## 逐章事件流` **追加**一行 `| 第N章 | 角色 | 维度 | 变化 |`，同时把该角色的当前态写进写作简报的 `## 在场角色`。日更不修改上方快照块——那需要先读整个文件，而它会长到上百 KB
3. 如果无变化：跳过
4. 仅追踪主要角色（出场 ≥3 次或有独立剧情线的角色），不追踪路人角色
5. 快照块重建：`/story-review` 或导入时按 `## 逐章事件流` 重算各角色当前态，重算后可清掉已并入字段的旧事件行。日更不做这件事
