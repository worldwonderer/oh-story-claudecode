...
function buildTask(m, mode) {
  ...
  // 写手 agent 模板是长篇/短篇通用的，它的参考表指向通用 agent-references。作品类型
  // 自己的写作规范（短篇默认第一人称、双轨切换、题材公式……）全在 skill 的 references
  // 里，宿主会话本来是加载了那个 skill 才动笔的，委派方一点都看不到。实测短篇因此写
  // 成了第三人称，而 story-short-write 明文规定「除非题材明确需要，否则一律用「我」」。
  body += line('作品类型写作规范（**最高优先级，落笔前必读**；与通用参考冲突时以它为准）', m.skill_references)
...
**长度纪律**
目标字数是**要瞄准的数**：既不要为了写透而超出，也不要为了收敛而写不足。
- 落笔前先按目标字数把细纲情节点分配好篇幅，超出的部分靠**减少铺陈**解决，不要靠删情节点。
- 环境描写、器物细节、旁观者反应各自只保留最有功能的一处；同一信息不要在叙述和台词里各说一遍。
- 接近目标时收束进结尾钩子，不要因为"还没写透"继续展开。
- 离目标还差得远，说明已有情节点没演够：回去把它们展开成有动作、对话与具体物件的场景，**不要新增情节点，也不要注水**。

**执行**
1. **先读「作品类型写作规范」列出的每一个文件**——那是本作品类型的硬约束（叙述人称、腔调、结构公式、格式），优先级高于你自己的参考文件体系表；两者冲突时以它为准。
2. 再按你的参考文件体系表逐行独立判定，命中即用只读工具读取该参考文件；角色档案与设定档案一并读，人物性别、身份与口吻以档案为准，不要自行设定。
3. 写出完整正文（含标题行），长度守住上面的纪律。
`
