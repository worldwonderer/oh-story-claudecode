...
精修后对 `正文.md`，**先**运行 `node scripts/check-ai-patterns.js --check 正文.md` 报告六类需按语义/功能改写的问题：① 高危 AI 对比句式（删否定铺垫直接写后项）、② 破折号（按功能改写：打断→动作/短句，拖长音→省略或动作，插入说明→逗号/冒号，**不要一律改句号**）、③ 碎句号（连续短叙述句无呼吸→按目标句长合并成中长句）、④ 长段落（>200 字→按镜头/动作/视线断段）、⑤ 微动作复读（「了下/了一下」式轻反应高密度→合并动作、换具体细节）、⑥ 抽象总结复读（命运/棋局/这一刻终于明白/才刚刚开始→回到角色当下证据）；①② 回正文改掉、复扫到 0，③④⑤⑥ 是提示按情况改。**再**运行 `node scripts/normalize-punctuation.js 正文.md`（写模式，默认 `--quote-mode keep`）机械兜底：清除残留 `……`、漏网破折号 `——`/`—`、双连字符 `--`、独立行 `---`；数字区间不设例外；盐言「」引号不受影响。
