...
> 如果用户有参考小说，先用 `/story-short-analyze` 拆解，默认输出存入项目根目录 `拆文库/{书名}/`（或用户指定的 `对标/{书名}/` 目录）。写作时会自动查找并读取这些拆文结果，不需要用户手动复制到 prompt。

#### 对标上下文加载

> **拆文库/对标关系**：`拆文库/` = analyze skill 的原始产出（source of truth）。`对标/` = 写作项目的引用视图。短篇写作优先读取 `{短篇标题}/对标/{书名}/`，不存在则回退项目根 `拆文库/{书名}/`，再回退 `{短篇标题}/拆文库/{书名}/`。

如果工作目录下存在 `对标/` 或 `拆文库/`，或用户提到参考小说：
...
**拆文结果自动使用规则**：执行写作前必须扫描 `对标/` 与 `拆文库/`。找到拆文报告时，把“结构/情绪/反转/写作手法”作为技法参考；找到结构化子目录时，按当前小节目标检索最相关模块。
