...
> **对标节奏回流（有主对标书时，卷纲定稿前做一次）**：读主对标 `对标/{书}/剧情/节奏.md`；缺失时设置 `missing_primary_contract: true` 并停止，提示重跑 `/story-long-analyze` Stage 3+ 或重新 `/story-import`，不得用章节摘要、故事线或拆文报告代替。文件存在时，按 references/outline-structure-theory.md「对标节奏迁移」把对标的一级结构关键点（1/4·中点·3/4）换素材排进本卷卷纲「对标结构坐标」——选段以主对标剧情单元为单位，按「类型/桥段标签」圈同类（详见该节步骤 1），选中的剧情单元写入剧情单元卡「对标剧情参照」。**「对标剧情参照」允许填「自创：{一句话理由}」**——对标坐标只锚 1/4·中点·3/4 三个节奏点，单元的内容形态不强制映射对标；连续两个单元同场景同形态时（供给自查第⑤行）自创正是首选出路。自创单元照常过大纲安全七检与批末结构复核，不额外加审。若尚未登记主对标书但 `拆文库/` 有同题材候选，先回 Phase 1「对标发现」登记再排节奏；确无对标书则按八节点占比自排。
...
3. **供给自查**（每批出细纲前必做，产出《供给单》）：对本批章节答五行——①**对手**：每章谁站在主角对面？具名对手断档超 2 章即处置（调度既有角色或按 B 级发明）。②**角色**：3 章内出场的具名角色有无 `设定/角色/{名}.md`？没有的先建卡。③**能力/金手指**：增量落在哪章、既有库存接不接得上？④**刻度**：本批要引用的每个数字、制度、规矩，**当场 grep 验证已在设定中存在**——「数字照 X.md」写下之前先确认 X 里真有这组数，不许把槽位指向查无实据的权威文件。⑤**场景**：与上一单元同场景同对手同胜负手连排则至少换掉一项。《供给单》以小节「供给自查」落当前卷纲该单元卡下：B 级项各一行「发明了什么＋落在哪个设定文件」；C 级项列 1-2 个候选提案＋影响面，批末一次交作者裁定；无缺口也落「无缺口」一行。上一批写手申报的「想要没拿到」列是本步固定输入。
...
2. **B 级·发明＋落档**（具名配角、能力名与形态、物价内插、规矩细则、可数刻度序列——凡在既有力量体系/经济表/制度框架**之内**、且不与既有裁定冲突的）：**直接发明**。能从已有内容推的（已有八格身份阶梯、只是没写各格待遇）照推；推不出但框架装得下的，挑一个不与任何既有条目打架的值写死。先增量补进对应设定文件，**再**在细纲里引用（顺序固定：先落档后引用），并在本批《供给单》登记一行。
3. **C 级·提案裁定**（金手指规则、真相层、伏笔、主线事件、力量体系档位、经济锚点数字、读者契约与马甲/归因相关——动了牵连全书的）：不发明、也不拿 `[待补充]` 了事。在《供给单》列 1-2 个候选方案＋各自对既有设定的影响面（grep 到的关联条目），批末一次交作者裁定；裁定先落设定文件，细纲再引用。缺口写成具体问题（"资质一共几档？凡骨在第几档？"），不临时编数进细纲。
...
- **大纲/卷纲_第X卷.md**：每卷的剧情单元+情绪弧线（含章节定位）+人物弧线+伏笔+反转+对标结构坐标+**刻度台阶/打脸债/压力线轮转**三表（参考 outline-methods.md「大纲三层结构法」 + outline-structure-theory.md「章节定位与张弛 / 对标节奏迁移」 + emotional-arc-design.md「六种弧线速查」 + long-reversal.md「反转类型」；三表模板见 artifact-protocols.md）
