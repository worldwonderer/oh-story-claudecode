...
> **对标节奏回流（有主对标书时，卷纲定稿前做一次）**：读主对标 `对标/{书}/剧情/节奏.md`；缺失时设置 `missing_primary_contract: true` 并停止，提示重跑 `/story-long-analyze` Stage 3+ 或重新 `/story-import`，不得用章节摘要、故事线或拆文报告代替。文件存在时，按 references/outline-structure-theory.md「对标节奏迁移」把对标的一级结构关键点（1/4·中点·3/4）换素材排进本卷卷纲「对标结构坐标」——选段以主对标剧情单元为单位，按「类型/桥段标签」圈同类（详见该节步骤 1），选中的剧情单元写入剧情单元卡「对标剧情参照」。**「对标剧情参照」允许填「自创：{一句话理由}」**——对标坐标只锚 1/4·中点·3/4 三个节奏点，单元的内容形态不强制映射对标；连续两个单元同场景同形态时（供给自查第⑤行）可用自创。自创单元照常过大纲安全七检与批末结构复核，不额外加审。若尚未登记主对标书但 `拆文库/` 有同题材候选，先回 Phase 1「对标发现」登记再排节奏；确无对标书则按八节点占比自排。
...
3. **供给自查**（每批出细纲前必做，产出《供给单》）：对本批章节答六行——①**对手**：每章谁站在主角对面？具名对手断档超 2 章即处置（调度既有角色或按 B 级发明）。②**角色**：3 章内出场的具名角色有无 `设定/角色/{名}.md`？没有的先建卡。③**能力/金手指**：增量落在哪章、既有库存接不接得上？④**刻度**：本批要引用的每个数字、制度、规矩、**距离与脚程**，**当场验证已在设定中存在，并把结果写进《供给单》该行**（「X 已在 A.md 某组确认」）——不写等于没验；「数字照 X.md」写下之前先确认 X 里真有这组数，不许把槽位指向查无实据的权威文件。⑤**场景**：两问——本批剧情**用到具名地点的空间属性**（距离/方位/进出口/内部分区任一）时，该属性落档没有？没有的按 B 级当场补进地理文件再排纲；只被提名、不消费空间属性的地点不强制建底图；与上一单元同场景同对手同胜负手连排则至少换掉一项。⑥**线**：本批除主线外有哪几条 L 线在动、各推到哪一步？全批只有主线在动即处置（调度卷纲既有 L 线，或按 B 级新开一条并追加进「本卷故事线」表）——主线薄的章靠这一行给的存量顶，不靠拉长主线。《供给单》以小节「供给自查」落当前卷纲该单元卡下：B 级项各一行「发明了什么＋落在哪个设定文件」；C 级项列 1-2 个候选提案＋影响面，批末一次交作者裁定；无缺口也落「无缺口」一行。上一批「本章没写成的」里挡住原因为「细纲没有这个点」的条目、上一批《供给单》的「建纲追加」小节（步骤 5），以及 `grep -rn "待裁定" 大纲/` 捞出的未结三档项，是本步固定输入。
   **本步只收批前预见的缺口**；当场发现的走步骤 5，不回填进本小节。
...
- 镜头准入：{按本章实际分配，一人一档，同一人不同章可以换档、也可以兼档——台词位（有台词、有自己在争的东西，一场戏 3-4 人封顶）/ 动作位（只给一个动作或一个目光，不给台词）/ 背景位（合并成一句集合描写，不点名）。给了台词位的人要写清他此刻在保什么、在算什么；答不上来的降到动作位}
- 人物关系变化：{本章前 → 本章后}
...
| 1 | {主角在账单上发现4800元转出} | {信息揭示} | {密} | {禁：不揭露幕后者。放：他当场算出这笔钱等于几个月房租，允许写出来} |
  ...
  **分辨率＝这一点给多少分辨率，不是给多少字**：密（慢镜头逐拍展开）/ 中 / 疏（一句带过）。**全章不许全填「中」**；同类并列的点递减、不平均。留空按「中」处理。
...
1. **A 级·自由发明**（**一次性**场景陈设、路人、一次性对话、习俗细节）：直接写进细纲，不落档、不上报。判据是**这地方后面还来不来**（不来的归本级，会再来的归 B 级底图，不按品类划）。
2. **B 级·发明＋落档**（具名配角、**具名地点的底图**、能力名与形态、物价内插、规矩细则、可数刻度序列、**新故事线**——凡在既有力量体系/经济表/制度框架**之内**、且不与既有裁定冲突的）：**直接发明**。能从已有内容推的（已有八格身份阶梯、只是没写各格待遇）照推；推不出但框架装得下的，挑一个不与任何既有条目打架的值写死。先增量补进对应设定文件，**再**在细纲里引用（顺序固定：先落档后引用），并在本批《供给单》登记一行。新故事线的落档处是卷纲「本卷故事线」表——追加一行、分配 L 编号、当场写出它与主线的对接口和「能否单独扛一章」，落表后细纲才引用。具名地点的落档处是 `设定/世界观/地理.md`（或该书同位文件）——首次消费某项空间属性前把**用到的那几项**（范围/分区/进出口/相对位/**脚程**）落档，未用到的不强制补全、后续用到再增量补；落档后细纲与正文才引用。**有专名即复现**（起了名字的地点按 B 级落档，不作一次性处理）。
3. **C 级·提案裁定**（金手指规则、真相层、伏笔、主线事件、力量体系档位、经济锚点数字、读者契约与马甲/归因相关——动了牵连全书的）。另有两类同归 C 级：
   - **推翻既有设定里已经写死的东西**——角色卡上「他的规矩」「他的动机」这类行为裁定、既有数值、既有制度细则。**为了避开与前文形态复读而给某个角色破一次例，就是推翻**：破例要么变成设定（补进卡里，写清例外条件），要么不破。不许只活在某一章的细纲里。
   - **同一件事两个设定文件写得不一样**（含卷纲与设定文件冲突）。**不许挑一个顺手的照办然后往下写**——哪一个作准是作者的裁定，列出两处出处与各自影响面交上去。
   
   这两类不发明、也不拿 `[待补充]` 了事。在《供给单》列 1-2 个候选方案＋各自对既有设定的影响面（grep 到的关联条目），批末一次交作者裁定；裁定先落设定文件，细纲再引用。缺口写成具体问题（"资质一共几档？凡骨在第几档？"），不临时编数进细纲。
...
- **大纲/卷纲_第X卷.md**：每卷的故事线（L 编号，主线之外至少 2 条）+剧情单元+情绪弧线（含章节定位）+人物弧线+伏笔+反转+对标结构坐标+**刻度台阶/打脸债/压力线轮转**三表（参考 outline-methods.md「大纲三层结构法」 + outline-structure-theory.md「章节定位与张弛 / 对标节奏迁移」 + emotional-arc-design.md「六种弧线速查」 + long-reversal.md「反转类型」；三表模板见 artifact-protocols.md）
