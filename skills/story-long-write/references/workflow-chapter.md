...
7. **正文执行**：
   - **正文外包（默认关闭）**：读 `.story-deployed` 的 `prose_delegate`；缺失 / 空 / `none` 就跳过本条，按下面原流程写。为 `agy` 时改按 `references/prose-delegate.md` 执行，其余步骤不变。
...
8. **非对称收口**：运行 `{PYTHON} {skill 根}/scripts/storyctl.py chapter check --project {项目根} --chapter {N}`；文件变更重跑，不存决议。
   ...
   - `over` + pass → 删除区间随 `compress-once` 交 narrative-writer；一次净删，零新语义/契约改动。走了外包分支时见 `references/prose-delegate.md`。会话核对情节点与钩子后复检；带内提交，否则上述三动作。
