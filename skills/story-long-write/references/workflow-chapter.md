...
| **③ 提请作者确认** | **需要在卷纲占编号或占行**（新伏笔、新故事线、新反转、新打脸债）；或碰卷纲主推线/终局底牌边界、与活跃伏笔或下章承诺可能打架、或 S3 但需创作决策 | 本章报告里列出（新增物 + 后续义务 + 撞上哪一条）交作者定夺，同时在 `大纲/细纲_第XXX章.md` 末尾「#### 本章新增物处置」小节挂一行 `品类｜名目｜落在正文哪处｜状态：待裁定`。**正文保留原样，该项本轮不写进事务 delta** |

- **拿不准就归三档，不自动收编。**
- 三档项**不进 tracking**（不派生到伏笔视图、时间线、角色快照和状态卡）。作者确认收编的：**先**按补纲授权把行追加进卷纲对应表（F／L 编号在此分配），**再**走 `mode=revision` 同章事务登记，最后把「本章新增物处置」那行的状态改成 `已收编` 或 `已删`（顺序固定）。未裁定项留在细纲末尾挂账，下一批排纲时由供给自查第 3 步的固定输入捞出。
