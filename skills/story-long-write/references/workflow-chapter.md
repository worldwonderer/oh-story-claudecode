...
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   - **第 0 步：先跑组装脚本**（可用时）。`{PYTHON} {skill 根}/scripts/build_writer_prompt.py --project {项目根} --chapter {N} --out {留档文件}`，Windows 下加 `PYTHONIOENCODING=utf-8`。stdout 的 `====` 分隔线以上是 prompt 正文、以下是核对报告（注入了哪些块、跳过哪些、为什么，以报告为准）。脚本已做完的部分下面不再重做，主会话只填空槽（搜 `［主会话填］`）。退出码 2 ＝ 数据问题，先修数据再重跑，不得手拼绕过；脚本跑不起来才按步骤 7 回落。
...
7. **正文执行**：
   ...
   - **回落分两种**：①脚本**跑不起来**（python 缺失、脚本报错、用户指定不走脚本）→ 主会话接管脚本那九项，按下列规格逐条传，报告里声明 `Fallback: build_writer_prompt -> 手动组装`。②脚本**退出码 2**（细纲缺失、续写状态卡缺栏、上一章找不到）→ 不是回落，是停下修数据，修完重跑脚本。
   - 手动组装规格如下（漏传等于没写）：
   ...
   - 第二行起传本章必需材料：
     - 项目目录、章节、细纲文件（**只给路径，不给全文**）、上一章结尾切片（按整行回退约 400 字，**不给路径**）、输出路径与标题行字面量（全反斜杠，章名从细纲 `### 第 N 章：章名` 解析，标题行形态照既有章）。
     ...
     - **agent 定义里已有的不重述**，只留一行指针：细纲优先边界＝铁律 1、正文形状＝铁律 2、新增物三档＝铁律 3、阅读体验字段（本章标价／闭环状态／信息差触发点／镜头准入档位／情节点分辨率列／执行边界「放」半边）＝铁律 4、交付三附件（时空表／新增申报表含末行「本章想要但没拿到的」／参考文件读取清单）＝被调用协议。申报表与「想要没拿到」行的去向见步骤 12 与 workflow-setup「按剧情批出细纲」步骤 3。
...
8. **先核时空表，再非对称收口**：
   - **核时空表**（交付确认）：细纲每个情节点是否**至少出现一次**——有漏即退回补写（补的是批准内容，不是加戏）；横跨多个时空的点须标注，每一处都要有实在内容。若时空表基本是「一个时空一个点」，说明子代理按细纲顺序平推了，退回按形状层判据重织。这些退回属于覆盖/形状修复，不以字数为由发起。
...
| 档 | 判据 | 出口 |
|---|---|---|
| **① 收编** | 无 S1/S2，且不碰卷纲「本卷主推线/战果」「终局底牌边界」，不与 `## 活跃伏笔` / `## 下一章承诺` 打架 | 进步骤 12 那次逐章事务：角色/势力按 workflow-setup Phase 3 补建静态档案；可复用设定增量补进 `设定/世界观/{主题}.md`；伏笔进 `foreshadow_changes`；关系/状态进 `character_changes` + 角色快照；跨章约束进 `constraints`；下章要接的进 `next_chapter_commitments` |
| **② 修复** | S1 / S2 | 退回 writer 就地改正文或删掉该新增物。**限一次**（同「最多 2 次」口径），不开重试循环；改完若动了连续性事实，走下方 `mode=revision` 同章事务 |
| **③ 提请作者确认** | 碰卷纲主推线/终局底牌边界、与活跃伏笔或下章承诺可能打架、或 S3 但需创作决策 | 本章报告里列出（新增物 + 后续义务 + 撞上哪一条）交作者定夺。**正文保留原样，该项本轮不写进事务 delta** |

- **拿不准就归三档，不自动收编。**
- 三档项**不进 tracking**（不派生到伏笔视图、时间线、角色快照和状态卡）。作者确认收编的，在下次逐章事务或 `mode=revision` 里补登记。
