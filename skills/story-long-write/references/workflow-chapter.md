...
8. **check 与用户收口**：运行 `{PYTHON} {skill 根}/scripts/storyctl.py chapter check --project {项目根} --chapter {N}`。长度在用户带内且 quality pass → `chapter commit`；带外且 quality pass → 展示证据和 `accept-current-length / revise-outline-or-target / discard`，接受时命令重读正文/目标、重数并立即原子提交；quality fail/invalid → 不提交。文件变化就重跑 check，不保存 approval/ID 链。
...
长篇每章只按本章细纲的 `字数目标` 与 `visible_chars_v1` 口径测量：内部质量带 ±12%，用户带 ±15%。节奏类型只决定展开方式，不叠加静态最低字数；细纲缺目标时报告 `invalid`，不设 3000 字兜底。长度状态只负责测量；用户带内可正常提交，带外必须由用户接受当前长度或改目标/细纲/放弃。下一章只以 tracking 是否已提交为准。
