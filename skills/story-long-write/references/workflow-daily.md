...
当项目超过 30 章时，`追踪/上下文.md` 中的摘要应按三层结构组织，压缩远期上下文、保留近期细节：

| 层级 | 粒度 | 格式 | 维护时机 |
|------|------|------|----------|
| **近期层** | 近 5 章详细摘要 | 保留现有格式（每章一段：事件+状态变化+伏笔） | 每章写完后更新 |
| **中期层** | 每 10 章一段概要 | `章节范围 | 核心事件 | 状态变化` | 每写完 10 章生成一段 |
| **远期层** | 每卷一句话总结 | `卷名 | 主线进展 | 关键转折` | 每卷结束时生成 |
...
**token 节省估算**：100 章时，原方案需读 100 章上下文 → 新方案读 5 章详细 + 10 段中期 + N 句远期。预计 token 开销降低 60-70%（估算值）。
