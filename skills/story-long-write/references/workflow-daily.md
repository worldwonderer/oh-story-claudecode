...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - 写正文 → **字数验证（优先 Python 字符统计，`wc -m` 仅作 Unix 备选，< 目标90%则强制扩充）** → 检查钩子/爽点 → **正文元信息扫描** → 禁用词扫描
     - **正文元信息扫描**：标题行以外不得出现 `第[一二三四五六七八九十百千万两0-9]+章|上一章|上章|前一章|本章|这一章|前文|后文|伏笔|细纲|读者`。这些词属于写作/工程元信息，必须改成角色当下能感知的事件锚点或相对时间；例如“比第一章那三秒开火更疼”改成“比那三秒开火更疼”。只有角色在故事世界内真实阅读/讨论“第X章”文本时例外。
   - 每章写完后**立即更新**以下文件：
     - `追踪/伏笔.md`（新增/回收伏笔）
     - `追踪/时间线.md`（记录事件时序）
...
3. **正文元信息扫描**：检查标题行以外是否混入 `第X章`、`上一章`、`本章`、`前文`、`伏笔`、`细纲`、`读者` 等写作工程词，命中即改成场景内表达
4. **钩子检查**：每章章尾是否有钩子
