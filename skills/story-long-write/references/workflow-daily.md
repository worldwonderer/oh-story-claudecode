...
>
> 写前资料清单、对标书路径查找、题材正文提示卡、自定义文风模式、文风缺失 fail-fast、模块/节奏缺失停止、权威冲突规则、无对标项目处理，一律按 `SKILL.md` Phase 4 的「缺失文件处理」「对标分析权威优先级」和写前准备 (a)-(g) 执行；本文件不另立一套。读者契约、主角代理权、期待债、终局储备参 `reader-contract-and-progression.md`。
>
