...
| 序号 | 文件 | 用途 | 如果不存在 |
|------|------|------|-----------|
| 1 | `追踪/上下文.md` | 写作状态摘要（≤12KB，整份读）；通常的日更状态都在这里，固定待处理项超过 5 项时按下方例外加载 | 用本章细纲 + 最近 3 章正文按「状态摘要格式」新建 |
...
**固定待处理项超限时**：如果状态摘要 `## 历史记录索引` 标明有溢出待处理项，或 `追踪/逐章记录/_待处理事项.md` 存在，本章必须读取该文件，与 `## 下一章必做事项` 合并后按目标章节选出最早 5 项。这是唯一的额外必读例外；不得借此读取其他逐章记录。
...
| `## 长期约束` | ≤6 行 | **固定保留，不得按写入顺序自动删除**。只有用户明确撤销才能删。第 7 条出现时，先把新裁定完整记入本章逐章记录，然后停止下一章写作并请用户指定合并方式；不得删除、擅自合并或写出超限的状态摘要 |
| `## 在场角色` | ≤6 人，每人 1 行 ≤60 字 | 每次完整重写。格式 `角色名｜身份｜当前状态（第N章变化）` |
...
| `## 近三章速记` | 恰好 3 条，每条 ≤80 字 | 每次完整重写。出现第 4 条时删除最旧一条 |
| `## 下一章必做事项` | ≤5 行 | 每次完整重写。默认只放下一章要处理的一次性任务；跨多章才能结清的行首标 `[固定]` 并写明目标章节。通常替换时先移出最早写入的非固定项；固定项超过 5 项时，**按目标章节升序只保留最早的 5 项**，其余当前待处理项写入 `追踪/逐章记录/_待处理事项.md`，不得丢弃 |
...
| `## 待办` | ≤5 行 | 每次完整重写 |
| `## 历史记录索引` | ≤6 行 | 每次完整重写。列出 `追踪/` 下各历史记录文件的用途与当前行数；`_待处理事项.md` 存在时必须列出待处理项数量 |
...
- **固定待处理项超过 5 项**：用 `Write` 整份重写 `追踪/逐章记录/_待处理事项.md`，只保留未完成的溢出项，格式为 `| ID | 事项 | 目标章节 | 来源章节 |`。每章重建状态摘要前读取该文件，将目标章节最早的项补入前 5 项，已完成项删除；并在 `## 历史记录索引` 记录剩余数量。
...
1. `grep -A8 -E '^## (长期约束|累计待处理项|最近决策|待处理线索)' 追踪/上下文.md` —— **这一步不能省**。永久裁定在文件靠前的位置：新格式在 `## 长期约束`，v21 及更早的项目在 `## 最近决策`（当时的规则就要求写在那里）、开放线索在 `## 待处理线索`，下一步的 `tail` 都取不到，而本章细纲和最近 3 章正文里也没有它们。漏掉就等于把用户此前所有「对全书永久生效」的裁定一次性作废。取到后分别保存：永久裁定进新状态摘要 `## 长期约束`，累计待处理项原样带进新状态摘要（这是累计数值，丢了只能读遍全部逐章记录重算），仍开放的线索进 `## 活跃伏笔` 或 `## 下一章必做事项`，已落地的旧决策不迁移（正文即证据）。
2. `tail -c 8000 追踪/上下文.md` 取尾部近况。
3. `mkdir -p 追踪/逐章记录`，然后把整个旧文件**改名**成 `追踪/逐章记录/_旧上下文_截至第{N}章.md`（`mv` / Windows `move`，一次操作、不读内容、无编码风险、原文一字不删）。改名失败就停下报告，不得在原文件还在原地时执行下一步的 `Write` 覆盖。
4. 用第 1 步分类后的内容 + 第 2 步的尾部 + 本章细纲 + 最近 3 章正文，`Write` 一份符合上表 11 区块的新状态摘要。`追踪/伏笔.md`、`追踪/时间线.md`、`追踪/角色状态.md` 不动——它们已不在读路径上。

**与项目 rules 的关系**：部分项目的 `.claude/rules/story-consistency.md` 写有「设定决策记录到 上下文.md 的最近决策部分」。以本节为准：永久生效的进 `## 长期约束`，影响后续 3 章内的进 `## 下一章必做事项` 并标 `[固定]`，其余记进本章 `追踪/逐章记录/第{NNN}章.md` 的「决策」行，不新建 `## 最近决策`。
...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - **状态筛选**：每章开始前必须确认以下三项通常来源已经在本轮 workflow 中读取或刚更新：本章细纲、上一章正文（或上一章刚写入的正文）、`追踪/上下文.md`（状态摘要）。"已加载"只指本轮 workflow 内实际读取/更新过的文件，不得用未标明来源的聊天记忆替代。角色最新状态从状态摘要 `## 在场角色` 取，待回收/推进伏笔从状态摘要 `## 活跃伏笔` 取；固定待处理项超过 5 项时，还必须读取 `追踪/逐章记录/_待处理事项.md`。状态摘要里没有的其他内容按下方「旧信息查找步骤」按需查询，**除上述专用待处理文件外，不得 Read `追踪/伏笔.md`、`追踪/时间线.md`、`追踪/角色状态.md`、`追踪/逐章记录/` 全文**——那会让每章的读取量重新涨回去。细纲不存在时仍按下方补建流程处理，不允许直接写正文
   ...
   - 每章写完后**立即分两步更新追踪文件**，顺序不可颠倒：
     ...
     **第一段：记录本章**。用 `Write` 新建 `追踪/逐章记录/第{NNN}章.md`（三位补零，如 `第031章.md`；章号过千按实际位数），这份文件完整记录本章内容，后续以它为准；必须长期保留，日更不读取：
     ...
     写完 `追踪/逐章记录/第{NNN}章.md` 后，再把记录行 `>>` 追加进三个历史记录文件。**追加位置固定是各文件末尾的 `## 逐章更新记录` 小节**——它必须是文件最后一个小节，这样 `>>` 追加的行会落在该小节中。该小节不存在时（旧项目）先 `>>` 追加一行小节标题再追加数据行；若发现它不在文件末尾（被手工编辑过），先在文件末尾重建一行小节标题再追加。没有这个小节就直接 `>>` 的后果：三个模板原本的最后一张表分别是「过期伏笔」「待确认（时间线疑点）」和最后一个角色小节，新伏笔会被记成过期、角色变更会记到错的角色名下。
     ...
     这三个文件**只用 shell 追加重定向，不得用 Read / Edit / MultiEdit**——Edit 要求先 Read 全文，对已长到上百 KB 的历史记录文件会让每章的读取量重新涨回去。追加失败不阻塞：完整记录已在 `追踪/逐章记录/` 里，`/story-review` 可据此重建。

     **第二段：重写状态摘要**。`Write` 整份覆盖 `追踪/上下文.md`（本轮 Step 1 已读过它，≤12KB，覆盖不需再读），按上方「状态摘要格式」的 11 个区块与条数上限重建。一般条目移出状态摘要时，完整内容已在本章或更早的逐章记录中；活跃伏笔与固定待处理项则必须分别按上方规则留在 `追踪/伏笔.md` 或 `追踪/逐章记录/_待处理事项.md`，不得直接丢弃。
...
查询结果通常不写入状态摘要；只有该条在本章之后仍然有效时，才写入 `## 活跃伏笔` 或 `## 在场角色`，并按各自的上限规则删除一条旧项。
...
6. **伏笔盘点（仅本轮增量）**：只确认本批新增/推进/回收的伏笔的追加行已落在 `追踪/伏笔.md` 的 `## 逐章更新记录`（`tail -20 追踪/伏笔.md` 核对即可，不读全文、不原地改）；不得在日更流程中通读所有 session 或扫描全部正文做全量伏笔审计。全量伏笔审计只在 `/story-review` 或用户明确要求"全面检查伏笔"时执行
...
**本步不再写任何内容进状态摘要**——每章 Step 2 已经整份重建过它。只做三件事：
...
3. **长周期概要**：章号能被 10 整除时，往 `追踪/阶段摘要.md` 追加一段十章概要（文件或 `## 十章概要` / `## 卷级总览` 小节不存在时，先 `>>` 追加小节标题再追加数据行）（`章节范围 | 已写核心事件 | 角色状态变化`），并把同一行写进状态摘要 `## 十章概要`（只保留最近 4 行，超过时删除最旧一条）；卷末追加一行卷级总览（`卷名 | 已写主线进展 | 关键转折`）。该文件日更不读，供用户和 `/story-review` 回看全书进展。

然后向用户口头汇报本批完成情况（章数、字数、漂移、下一批建议），不另存为文件。

---

> **漂移处理**：aligned = 按计划推进；adaptive = 细节适配但不改契约；structural 漂移 / 结构性漂移 = 正文已经改变卷契约、单元承诺、推进线或兑现归属，必须修正文或重规划未来章节细纲，不能只在逐章记录里备注。未解决的 structural 漂移在状态摘要 `## 下一章必做事项` 占一行并标 `[固定]`，修掉后才删；固定项总数超过 5 项时按「超出上限时的处理」写入 `_待处理事项.md`，不得让状态摘要超限。
