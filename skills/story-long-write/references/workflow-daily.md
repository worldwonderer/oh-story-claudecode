...
1. **读取写作计划**：写作计划权威顺序为 **卷契约 → 当前循环 → 章节细纲**；已发生事实仍以正文与 `追踪/` 文件为权威。先从 `大纲/卷纲_第X卷.md` 读取卷契约、当前循环（循环ID/位置）、循环情绪引擎、本卷主推线/战果与终局底牌边界，再加载本次要写的 2-3 章细纲。旧版卷纲缺少卷契约/循环卡不阻塞日更：本轮在内存推断并记录到 `追踪/上下文.md`；仅在明确补纲/改纲时回写卷纲；未知字段写 `[待补充]`；绝不自动修改已锁定卷纲。新版细纲优先读取「阶段位置」「本章结构公式」「本章禁止提前释放」「内容概括（起因/发展/转折/高潮/结尾）」「情节安排（主线/辅线/事件线/感情线/逻辑线）」「人物关系和出场顺序」「情节细化」「结尾设定和钩子」；旧版细纲缺这些字段不阻塞，回退到核心事件、情节点序列、目标情绪、章首/章尾钩子、字数目标。旧细纲有字数目标但无逐点预算时，写后定位欠账先按功能标签派生临时预算（爽点/打脸/反转=密、过场/赶路=疏）再分配，不凭感觉挤牙膏。
   - **批次定位与阶段约束**：写本批前先从 `大纲/大纲.md`、对应 `大纲/卷纲_第X卷.md` 和本批细纲提取：当前章节区间属于哪个阶段、本批推进目标、本批可释放的信息、本批严禁提前释放的信息、章尾钩子不能越过的边界。必须按终局储备确认本批主推线与战果，别动用本阶段还不该解锁的终局底牌（多线齐涨的战果允许）；行动成本可无，不为征税硬造代价。没有明确阶段总览时，仅为本轮按卷纲和已写进度临时归纳；本轮明确补纲/改纲时才回写，否则记入 `追踪/上下文.md`；锁定大纲/卷纲绝不自动修改。
...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - 每章写完后**立即更新**以下文件：
     ...
     - `追踪/上下文.md`（只更新进度元信息：当前位置+已写字数+本次变更，不写详细角色状态/伏笔内容；复用既有计划节拍/实际结果/承诺进度等写后记录反映引擎推进，追加本章循环执行状态：循环ID、计划节拍、实际结果、收益变化、主推线/战果、承诺进度、漂移 aligned|adaptive|structural、下一章硬任务。不新增追踪字段，不创建单独记忆包或循环追踪文件）
