...
1. **选机制与曲线**：读主对标 `全局分析/爆款机制.md`、`全局分析/六维拆书.md` 的“三维节奏”章节与 `structure_blocks.csv`，按本卷读者需求和冲突类型选 1–3 个机制/结构块。跨书补充按 `cross-book-recall.md` 最多读取 2 本副对标的三个全局文件，不读副书原文。
...
建卡时做一次**剧情批召回**：从 `爆款机制.md` 提取触发链，从 `六维拆书.md` 的“三维节奏”章节和 `structure_blocks.csv` 提取峰前蓄力、峰值、冷却与完整循环；五列 `chapter_index.csv` 只把选中块的章号转成 locator。副对标只补充全局机制，只有主对标按 locator 读原文。结论写入剧情单元卡后，逐章细纲只消费剧情单元卡，不反复读取对标全书。
