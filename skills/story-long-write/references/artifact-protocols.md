...
- 追踪/逐章记录/第NNN章.md（逐章完整记录，日更只写不读）
- 追踪/阶段摘要.md（每 10 章一段概要 + 每卷一行总览，仅生成或修订对应概要时读取）
- 追踪/待处理事项/（每个未结清跨章事项一个不超过 300 字的小文件；状态摘要先保留普通事项和已到目标章的固定事项，再用剩余名额显示未来事项；每章最多读取 6 个）
- 追踪/伏笔.md
- 追踪/时间线.md
- 追踪/角色状态.md
- 对标/{对标书名}/拆文报告.md
...
## 逐章更新记录
<!-- 日更只往这里 `>>` 追加，不改上方三张表。当前状态 = 该 ID 在本小节的最后一行；回炉/改旧章的追加行按 workflow-revision 规则沿用当前状态，不得回退 -->
| ID | 伏笔内容 | 埋设章节 | 预计回收章节 | 状态{未埋/已埋/已回收/已过期} | 重要度{高/中/低} |
|----|---------|---------|-------------|-----------------------------|----------------|
```

**列序不能动**：SessionStart 的过期伏笔告警按第 6 列判状态，上下两处表列序必须一致。日更只追加 `未埋`/`已埋`/`已回收` 三种状态，`已过期` 由 `/story-review` 写入。
...
长周期概要，不在每章固定读取范围内。Step 4 只在章号能被 10 整除或到达卷末时，先 `Read`，再用 `Edit` 把数据写进对应表格；回炉时同样按章节范围或卷名改写既有行。两个表格共用一个文件，**不得在文件末尾直接追加**。

```markdown
# 阶段摘要

## 十章概要
<!-- 每 10 章一行；Read 后 Edit 到本表，已有起始章相同的导入残段或既有行时扩展/原地更新，禁止保留重叠区间 -->
| 章节范围 | 已写核心事件 | 角色状态变化 |
|---------|------------|------------|

## 卷级总览
<!-- 每卷一行；Read 后 Edit 到本表，已有相同卷名时原地更新 -->
| 卷名 | 已写主线进展 | 关键转折 |
|------|------------|---------|
```
...
每章一个小文件，完整记录该章内容，后续以此为准，日更只写不读。字段与写入时机见 `workflow-daily.md` 的「追踪文件分两步更新」。

---

## 追踪/角色状态.md

角色状态记录，Phase 3 大纲完成后创建初始状态，Phase 4 每章写完后按需更新。

模板字段、记录格式与更新规则统一见 `state-tracking.md`「角色状态记录格式」，本文件不再内联重复。
