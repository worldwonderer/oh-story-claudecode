...
- 细纲_第XXX章.md = 当前单章蓝图（阶段位置 + 结构公式 + 禁止提前释放 + 内容概括 + 多线情节安排 + 人物关系/出场顺序 + 情节细化 + 结尾设定/钩子 + 核心事件/目标情绪/章首钩子/爽点/章尾钩子/字数目标）
...
细纲是正文写作的章节蓝图，而不是只列事件和钩子的清单。所有写作入口都消费下列当前结构；字段不全时先补齐，未知字段写 `[待补充]`，不要为了补齐字段杜撰副线或人物关系。
