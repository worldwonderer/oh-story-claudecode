# 情绪弧线模板
> 情绪弧线设计速查：六种弧线、题材策略、期待感管理、情绪调动方法。
...
- 高点在 40-50%，反转后急速下坠，坠落比上升快，BE 不加糖
...
- 至少两个波峰一个波谷，通常三个起伏
- 每个峰比前一个高，最终峰在 80-90%，起伏间要有喘息，上限 3 个起伏
...
- 没有回落，像台阶一阶一阶往上，最高点在 85-95%，中间只给小胜利
...
- 前 70-80% 压/铺垫，最后 20-30% 集中爆发
- 铺垫不能无聊（每段有信息量和张力），压的方式多样，爆发密集，爆发后快收
...
- 前 70-80% 正常弧线，70-80% 处突然转向，急转只占 1-2 段
- 前面线索回看时必须说得通
...
1. **期待最大化定律**：读者需求最大的时刻 = 将要满足却还没满足的时候。爽点到来前那一刻是全篇张力最高位置。
2. **信息顺序操纵**：先植入什么信息，读者就先入为主往哪个方向想。
3. **期待递增法**：在基底期待上不断添加细节，注入更具体化的需求缺口。添加的细节必须与核心梗一致。
4. **断期待禁止**：下一个期待立起来之前，绝不能结束当前期待。旧期待即将兑现时，新期待的种子已经种下。
5. **下行情节安全原则**：写情绪下行时：锅都是别人的，功都是主角的。下行中给读者安全感（可能的解法/潜在收获）。
...
| 赌注类型 | 适用场景 | 要点 |
|----------|----------|------|
| 肉体死亡 | 恐怖诡异文 | 主角过强时赌注转为事业死亡，往往导致全书崩溃 |
...
- "不该如此"是最根本的情绪驱动力——读者看到诬告、不公、恃强凌弱，天生生出"不该如此"的念头
- 核心情绪逻辑：想要爽，必先不爽；想要公平，必先不公平；想要强大，必先不强大
- "悬而未决"也是强驱动力
- 好的题材往往具备勾起人们惋惜、渴望改变的特质
...
- **修饰词**：精准克制地使用形容词，不堆砌
- **最高境界**：通过行为暗示而非直接描写情绪；**以乐写悲**（赴死的人最悲的不是她哭，而是她很开心）
...
> 写完一段觉得不到位 / 读者反馈平淡时使用。理解所有情绪技法的本质——它们是同一台引擎的不同操作。
...
- **确定型**（爽文）：读者知道主角会赢，想看"怎么赢"（过程创意 > 结果悬念）
