...
| 5 | `追踪/上下文.md` | 写作简报：本章涉及的伏笔取 `## 活跃伏笔`、角色当前态取 `## 在场角色` | 简报缺项时 `grep` 冷文件取命中行，不读全文 |
...
1. **伏笔检查**：对比修改前后的伏笔列表，标记变化项。冷文件是只追加的，一律往 `追踪/伏笔.md` 末尾的 `## 逐章事件流` 追加新行表达状态变化，不做原地 Edit、不删旧行。**追加前先 `grep "{ID}" 追踪/伏笔.md | tail -1` 看该伏笔的当前状态**——回炉改的是旧章，但追加的行会排在后续章节的行后面、成为新的当前状态，所以除非本次改动真的改变了状态，追加行必须沿用查到的当前状态、只更新描述。例：F018 已在第 38 章回收，回炉第 12 章只改它的描述 → 追加行状态仍写 `已回收`，绝不写 `已埋`，否则一条收过的伏笔会重新进简报被二次回收：
   - 新增伏笔 → 追加一行，状态 `已埋`
   - 删除伏笔 → 追加一行，状态 `已回收` 并在内容里注明「改章后作废」，同时检查后续章节是否引用该伏笔
   - 修改伏笔描述 → 追加一行新描述，状态沿用 grep 查到的当前状态
   - 回炉真的重开了一条已回收的线（剧情改了）→ 才写 `已埋`，并在简报 `## 活跃伏笔` 登记
   - 各种情况都要同步改简报 `追踪/上下文.md` 的 `## 活跃伏笔`
2. **时间线检查**：对比修改前后的事件顺序，往 `追踪/时间线.md` 的 `## 逐章事件流` 追加修正行
