...
4. 如果角色、普通剧情线或设定子目录存在，写作时按需召回相关模块。
...
> **对标节奏回流（有主对标书时，卷纲定稿前做一次）**：读主对标 `对标/{书}/剧情/节奏.md`；缺失时设置 `missing_primary_contract: true` 并停止，提示重跑 `/story-long-analyze` Stage 3+ 或重新 `/story-import`，不得用章节摘要、故事线或拆文报告代替。文件存在时，按 references/outline-structure-theory.md「对标节奏迁移」把对标的一级结构关键点（1/4·中点·3/4）换素材排进本卷卷纲「对标结构坐标」。无对标书则按八节点占比自排。
...
2. **角色、普通剧情线或设定等非主产物子目录缺失** → 按「对标书路径查找」查找项目视图与根目录数据源，仍缺失则跳过该可选模块。本条不适用于 `剧情/情绪模块.md` 和 `剧情/节奏.md`。
...
1. **检查细纲**：读取 `大纲/细纲_第{N}章.md`。如果不存在，或缺少当前“章节蓝图”的阶段位置、结构公式、禁止提前释放、内容概括、情节安排、人物关系/出场顺序、情节细化、结尾设定，**必须先补建/补齐细纲再写正文**。补建时参考卷纲中本章对应的事件规划和上下文；未知项写 `[待补充]`，不得跳过门禁直接写作。
...
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   ...
   - **指令确认**：综合细纲、本节速记和模块召回结果，用一句话写清本章意图。
	     - 当前细纲必须消费：阶段位置、结构公式、禁止提前释放、内容概括、情节安排、人物关系/出场顺序、情节细化、结尾钩子。
