...
**对标发现（主动推荐，先于下方反应式加载执行；用户已有方向也要做）**：无论用户是否点名对标书，先扫一遍已有拆解资产并按题材主动推荐，别等用户开口。

1. 列目录：`ls 拆文库/`（数据源）与项目 `对标/`（已引用视图），得到已有书目。两者都为空 → 跳到第 4 条。
2. 逐本读题材：短篇拆解读 `拆文库/{书}/_meta.json` 的 `genre_detected`；长篇拆解读 `拆文库/{书}/概要.md` 头部或 `拆文报告.md`「基本信息」的「题材」字段。与本书题材/方向（Phase 1 已确定的方向，或 `选题决策.md` 的推荐题材）比对，标记 同题材 / 弱相关 / 不相关。
3. 有同题材或弱相关候选 → 用 AskUserQuestion 主动推荐，不做开放式提问：「拆文库里有《X》（题材A）、《Y》（题材B）和本书相近，建议对标——主对标 1 本用于文风/正文，其余作副对标。用哪本？」选项含各候选书目 +「都不用（无对标，大纲自排）」+「我另拆一本再对标」。用户选定后：主对标书写入 `设定/题材定位.md`「对标登记 / 主对标书」字段，副对标追加进「对标书列表」（该文件在 Phase 2 生成，此处先记住选择、带入 Phase 2）；并按上方「首次引用对标书」规则把选中的书从 `拆文库/{书}/` 复制到 `对标/{书}/`。
4. 拆文库为空或无同/弱相关候选 → 明确告知用户当前无可用对标；想对标同类爆款可先 `/story-long-analyze` 拆一本再回来，无对标也可继续（大纲/卷纲按八节点占比自排，见 Phase 3）。
