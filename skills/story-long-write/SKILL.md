...
1. 定位当前卷、已写进度与既有剧情单元卡：读 `大纲/大纲.md`、当前 `大纲/卷纲_第X卷.md`、`追踪/上下文.md`（简报，待回收伏笔取 `## 活跃伏笔`、主角当前状态取 `## 在场角色`），以及 `设定/题材定位.md`（终局底牌与升级台阶）——简报缺项按 workflow-daily「历史检索路径」按需查询，不读冷文件全文；缺失的追踪文件按原流程推断，不阻塞；
...
5. 把「新剧情单元：{单元ID}（第A-B章，对标剧情参照：{…}）」「批次定位：{阶段/本批推进目标/禁释边界，一句话}」压成两行写进简报 `## 下一章硬任务`，完整推断记进当前章 `追踪/章记/第{NNN}章.md`；不新建简报区块。
...
│       └── 拆文报告.md
├── 追踪/
│   ├── 上下文.md                  ← 写作简报，≤12KB，日更每章整份读、整份重写
│   ├── 章记/第NNN章.md            ← 逐章权威记录，日更只写不读
│   ├── 卷史.md                    ← 十章概要 + 卷级总览，日更不读
│   ├── 伏笔.md                    ← 冷文件，日更不读，按需查询用
│   ├── 时间线.md                  ← 冷文件，日更不读，按需查询用
│   └── 角色状态.md                ← 冷文件，日更不读，按需查询用
├── 参考资料/
...
| 追踪/伏笔.md | 全书 | Phase 3 起 | 冷文件，日更不读；按需查询走 workflow-daily「历史检索路径」 |
| 追踪/时间线.md | 全书 | Phase 3 起 | 冷文件，日更不读；按需查询走 workflow-daily「历史检索路径」 |
| 对标/{书名}/拆文报告.md | 对标书 | 用户手动+analyze | Phase 2 核心设定、Phase 3 大纲、Phase 4 写作 |
| 追踪/章记/第NNN章.md | 章 | Phase 4 每章写完 | 日更不读；按需查询走 workflow-daily「历史检索路径」 |
| 追踪/卷史.md | 全书 | Phase 4 每 10 章 / 每卷末 | 日更不读；用户或 /story-review 回看全书进展时 |
| 追踪/上下文.md（写作简报，≤12KB） | 全书 | Phase 4 首次日更（workflow-daily 自动创建） | 日更每章整份读、整份重写；格式与体积见 workflow-daily「简报契约」 |
| 参考资料/{topic}.md | 按需 | Phase 4（story-researcher 输出） | Phase 4 后续章节写作时复用 |
| 追踪/角色状态.md | 全书 | Phase 3 | 冷文件，日更不读；角色当前态取简报 `## 在场角色` |
...
2. **读取上下文**（按需加载，缺失则跳过。可选快捷路径：如果项目已部署 story-explorer agent（优先检查 `.claude/agents/story-explorer.md` 是否存在；不存在时再检查 `.opencode/agents/`，再不存在时检查 `.codex/agents/`），可 spawn `Agent(subagent_type: "story-explorer", prompt: "项目目录：{dir}\n查询类型：context_load\n查询参数：准备写第 {N} 章")` 一次获取上下文）：
   ...
   - (3) `追踪/上下文.md`（简报）— 待回收伏笔取 `## 活跃伏笔`，角色当前态取 `## 在场角色`；日更不读 `追踪/伏笔.md` / `追踪/角色状态.md` / `追踪/时间线.md` 全文
...
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   - **状态筛选**：从简报 `追踪/上下文.md` 的 `## 在场角色` 取本章涉及角色的当前状态，从 `## 活跃伏笔` 取本章需要回收/推进的伏笔。输出本节速记（参考 state-tracking.md）。简报里没有的按 workflow-daily「历史检索路径」按需查询，不读冷文件全文；简报不存在时按 workflow-daily「简报契约」新建
   - **模块召回、题材卡与文风召回**：
...
12. **更新追踪**：按 workflow-daily「两段式写回」执行——先 `Write` 本章 `追踪/章记/第{NNN}章.md`（权威记录），再把事件行 `>>` 追加进 `追踪/伏笔.md` / `追踪/时间线.md` / `追踪/角色状态.md` 作检索索引（只追加、不 Read / Edit），最后 `Write` 整份重建简报 `追踪/上下文.md`。本章若首次引入会复用的具名角色/势力，按 Phase 3「细纲后设定补全」规则补建对应 `设定/` 档案。
13. **中途快照**（长篇写作安全网）：每连续写完 3 章，在继续前执行以下快照操作：
   - 确认简报 `追踪/上下文.md` 合规：`wc -c` ≤12288，顶层 `## ` 恰好是「简报契约」那 11 个、无重复标题
...
简报 `追踪/上下文.md` 每章整份重写、恒定 ≤12KB，不需要归档。全书历史在 `追踪/章记/`（每章一个小文件），长周期概要在 `追踪/卷史.md`，都不进日更读路径，按需 grep。伏笔、时间线、角色状态是只追加的冷文件，允许无界增长——它们不在每章读路径上，体积不产生每章成本。
...
检查后更新追踪文件（冷文件只往末尾 `## 逐章事件流` 追加，不做原地 Edit）：
- 往 `追踪/伏笔.md` 的 `## 逐章事件流` 追加过期或回收状态行
- 往 `追踪/时间线.md` 的 `## 逐章事件流` 追加时间线疑点的处理结果
- 若本轮改动了角色当前态或活跃伏笔，同步重写简报 `追踪/上下文.md` 的对应区块
