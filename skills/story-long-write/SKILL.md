...
1. 定位当前卷、已写进度与既有剧情单元卡：读 `大纲/大纲.md`、当前 `大纲/卷纲_第X卷.md`、`追踪/上下文.md`（状态摘要，待回收伏笔取 `## 活跃伏笔`、主角当前状态取 `## 在场角色`），以及 `设定/题材定位.md`（终局底牌与升级台阶）——状态摘要缺项按 workflow-daily「旧信息查找步骤」按需查询，不读历史记录文件全文；缺失的追踪文件按原流程推断，不阻塞；
...
5. 把「新剧情单元：{单元ID}（第A-B章，对标剧情参照：{…}）」「批次定位：{阶段/本批推进目标/不得提前写出的信息，一句话}」精简为两行写进状态摘要 `## 下一章必做事项`，完整判断过程记进当前章 `追踪/逐章记录/第{NNN}章.md`；不新建状态摘要区块。
...
- **追踪/伏笔.md** + **追踪/时间线.md** + **追踪/角色状态.md**：伏笔状态表+故事时间线+角色状态记录（参考 plot-core-methods.md「连续性追踪」、state-tracking.md「角色状态记录格式」）
...
│       └── 拆文报告.md
├── 追踪/
│   ├── 上下文.md                  ← 写作状态摘要，≤12KB，日更每章整份读、整份重写
│   ├── 逐章记录/第NNN章.md            ← 逐章完整记录，日更只写不读
│   ├── 待处理事项/第NNNNNNNN_源NNNNNNNN_SSSS.md ← 每个未结清跨章事项一个小文件，每章最多读 6 个
│   ├── 阶段摘要.md                    ← 十章概要 + 卷级总览，仅十章/卷末更新时读
│   ├── 伏笔.md                    ← 历史记录文件，日更不读，按需查询用
│   ├── 时间线.md                  ← 历史记录文件，日更不读，按需查询用
│   └── 角色状态.md                ← 历史记录文件，日更不读，按需查询用
├── 参考资料/
...
| 追踪/伏笔.md | 全书 | Phase 3 起 | 历史记录文件，日更不读；按需查询走 workflow-daily「旧信息查找步骤」 |
| 追踪/时间线.md | 全书 | Phase 3 起 | 历史记录文件，日更不读；按需查询走 workflow-daily「旧信息查找步骤」 |
| 对标/{书名}/拆文报告.md | 对标书 | 用户手动+analyze | Phase 2 核心设定、Phase 3 大纲、Phase 4 写作 |
| 追踪/逐章记录/第NNN章.md | 章 | Phase 4 每章写完 | 日更不读；按需查询走 workflow-daily「旧信息查找步骤」 |
| 追踪/待处理事项/第NNNNNNNN_源NNNNNNNN_SSSS.md | 事项 | 新增未结清跨章事项时 | 每项一个不超过 300 字的小文件；状态摘要只显示最早的事项，每章只读取脚本返回的最多 6 个路径 |
| 追踪/阶段摘要.md | 全书 | Phase 4 每 10 章 / 每卷末 | 仅生成或改写十章/卷概要时读取；用户或 /story-review 回看全书进展时 |
| 追踪/上下文.md（写作状态摘要，≤12KB） | 全书 | Phase 4 首次日更（workflow-daily 自动创建） | 日更每章整份读、整份重写；格式与体积见 workflow-daily「状态摘要格式」 |
| 参考资料/{topic}.md | 按需 | Phase 4（story-researcher 输出） | Phase 4 后续章节写作时复用 |
| 追踪/角色状态.md | 全书 | Phase 3 | 历史记录文件，日更不读；角色当前状态取状态摘要 `## 在场角色` |
...
2. **读取上下文**（按需加载，缺失则跳过。可选快捷路径：如果项目已部署 story-explorer agent（优先检查 `.claude/agents/story-explorer.md` 是否存在；不存在时再检查 `.opencode/agents/`，再不存在时检查 `.codex/agents/`），可 spawn `Agent(subagent_type: "story-explorer", prompt: "项目目录：{dir}\n查询类型：context_load\n查询参数：准备写第 {N} 章")` 一次获取上下文）：
   ...
   - (3) `追踪/上下文.md`（状态摘要）— 待回收伏笔取 `## 活跃伏笔`，角色当前状态取 `## 在场角色`；日更不读 `追踪/伏笔.md` / `追踪/角色状态.md` / `追踪/时间线.md` 全文
   - (3a) `<story-long-write>/scripts/select-pending-items.py --limit 6 --ordinary-count {普通事项数}` 的 JSON — 每章都运行以核对真实目录，并读取 JSON 返回的精确路径，最多 6 个；普通事项与已到目标章的固定事项合计超过 5 时停止，不重写状态摘要；目录是未结清跨章事项的完整依据
...
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   - **状态筛选**：从状态摘要 `追踪/上下文.md` 的 `## 在场角色` 取本章涉及角色的当前状态，从 `## 活跃伏笔` 取本章需要回收/推进的伏笔。每章先按 workflow-daily 运行待处理事项选择脚本，只读取 JSON 返回的精确路径，最多 6 个，并按 `record_id` 去重后重建摘要中的固定事项。输出本节速记（参考 state-tracking.md）。状态摘要里没有的其他内容按 workflow-daily「旧信息查找步骤」按需查询，不读其他历史记录文件全文；状态摘要不存在时按 workflow-daily「状态摘要格式」新建
   - **模块召回、题材卡与文风召回**：
...
12. **更新追踪**：按 workflow-daily「追踪文件分两步更新」执行——先 `Write` 本章 `追踪/逐章记录/第{NNN}章.md`（完整记录，后续以此为准），再把记录行 `>>` 追加进 `追踪/伏笔.md` / `追踪/时间线.md` / `追踪/角色状态.md` 作查询记录（只追加、不 Read / Edit），最后 `Write` 整份重建状态摘要 `追踪/上下文.md`。本章若首次引入会复用的具名角色/势力，按 Phase 3「细纲后设定补全」规则补建对应 `设定/` 档案。
13. **中途快照**（长篇写作安全网）：每连续写完 3 章，在继续前执行以下快照操作：
   - 确认状态摘要 `追踪/上下文.md` 合规：`wc -c` ≤12288，顶层 `## ` 恰好是「状态摘要格式」规定的 11 个、无重复标题
...
状态摘要 `追踪/上下文.md` 每章整份重写、恒定 ≤12KB，不需要归档。全书历史在 `追踪/逐章记录/`（每章一个小文件）；长周期概要在 `追踪/阶段摘要.md`，只在每 10 章、卷末或回炉改到对应范围时读取并更新。伏笔、时间线、角色状态是只追加的历史记录文件，允许持续增长——日更每章不读取它们，所以文件变大也不会增加每章的读取量。
...
检查后更新追踪文件（历史记录文件只往末尾 `## 逐章更新记录` 追加，不做原地 Edit）：
- 往 `追踪/伏笔.md` 的 `## 逐章更新记录` 追加过期或回收状态行
- 往 `追踪/时间线.md` 的 `## 逐章更新记录` 追加时间线疑点的处理结果
- 若本轮改动了角色当前状态或活跃伏笔，同步重写状态摘要 `追踪/上下文.md` 的对应区块
