...
**章节标题唯一性要求**（加载 `references/outline-methods.md`「章节名策略」）：
- 每章标题必须**唯一**，不得与已有任何章节标题重复或高度相似
- 标题必须反映本章独特内容，禁止使用过于通用的标题（如"意外""转折""危机"）
- 批量建纲时，建完后**必须**对全部章节标题做一次去重检查
- 相邻 10 章内标题禁止使用相同句式结构（如连续"XX的秘密""XX的真相"）
...
5.5. **章节标题唯一性检查**：确认本章标题（正文文件名中的章名）与已有所有章节标题不重复。如发现重复或高度相似，立即修改标题并重命名文件。参考 `references/outline-methods.md`「章节名策略」中的标题去重检查流程。
