...
| 追踪/阶段摘要.md | 全书 | Phase 4 每 10 章 / 每卷末 | 日更不读；用户或 /story-review 回看全书进展时 |
...
2. **读取上下文**（按需加载，缺失则跳过。可选快捷路径：如果项目已部署 story-explorer agent（优先检查 `.claude/agents/story-explorer.md` 是否存在；不存在时再检查 `.opencode/agents/`，再不存在时检查 `.codex/agents/`），可 spawn `Agent(subagent_type: "story-explorer", prompt: "项目目录：{dir}\n查询类型：context_load\n查询参数：准备写第 {N} 章")` 一次获取上下文）：
   ...
   - (3a) `追踪/逐章记录/_待处理事项.md`（如存在）— 只在 `[固定]` 待处理项超过 5 项时读取，与状态摘要合并后取目标章节最早的 5 项；不读取其他逐章记录
...
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   - **状态筛选**：从状态摘要 `追踪/上下文.md` 的 `## 在场角色` 取本章涉及角色的当前状态，从 `## 活跃伏笔` 取本章需要回收/推进的伏笔。如果 `追踪/逐章记录/_待处理事项.md` 存在，同时读取并合并目标章节最早的固定项。输出本节速记（参考 state-tracking.md）。状态摘要里没有的其他内容按 workflow-daily「旧信息查找步骤」按需查询，不读其他历史记录文件全文；状态摘要不存在时按 workflow-daily「状态摘要格式」新建
   - **模块召回、题材卡与文风召回**：
...
状态摘要 `追踪/上下文.md` 每章整份重写、恒定 ≤12KB，不需要归档。全书历史在 `追踪/逐章记录/`（每章一个小文件），长周期概要在 `追踪/阶段摘要.md`，日更每章都不读取，只在需要时用 grep 查找。伏笔、时间线、角色状态是只追加的历史记录文件，允许持续增长——日更每章不读取它们，所以文件变大也不会增加每章的读取量。
