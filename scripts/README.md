...
| `test-antigravity-skills-deploy.py` | Antigravity 14 个已知 Skill 原子物化、未知 Skill 保留、symlink fail-closed/显式迁移与防穿透写回归 | 被 `check-antigravity-adapter.sh` 调用 |
...
| `skills/story-doctor/scripts/story_doctor.py` | 部署与项目体检：部署完整性、运行时依赖、追踪结构、可选能力；三档严重度 + 三层修复（auto/prompt/refuse），`--fix` 只动受管文件 | story-setup 部署后 + 用户主动跑；被 check-story-doctor 调验 |
| `skills/story-long-write/scripts/delegate-prose.js` | 正文写作外包给外部 `agy` CLI：预检、组装 prompt、只读调用、解析 `--json-schema` 结构化产出、按宿主口径重数字数并记 usage | 仅 `prose_delegate: agy` 时由长篇/短篇写作流程调用；被 check-prose-delegate 调验 |
...
| `skills/story-setup/scripts/deploy-antigravity-skills.py` | 把 14 个 oh-story Skills 物化为项目 `.agents/skills` 真实目录；只替换已知名、保留用户 Skill，symlink 需显式迁移且不穿透写目标 | story-setup 部署时 + materialization 回归 |
