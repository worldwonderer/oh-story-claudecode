...
> 稳定版，三件事。一是补上主力端的闸：Claude Code 写正文时不检查追踪状态，另三端从 v0.7.3 起就检查，同一个工程同一次写入，主力端能静默写出若干章没有追踪的正文。二是长篇 skill 每次触发都整份进上下文的那份 SKILL.md 从 82 KB 降到 54 KB，开书三阶段抽成按需读的文件，日更会话不再为用不上的建纲步骤付钱。三是清掉一批过度累加的限制指令，其中一条正好把普通的「他说」判成违规。**本版 `agents_version` 为 24**（v0.7.4 是 23），已部署项目要重新运行 `/story-setup` 并新开会话。

### 修复

- **Claude Code 写正文不再漏掉追踪检查（#305）**。#283 给写正文守卫加了一整套追踪检查点：追踪状态缺失、schema 不是当前版本、续写状态卡的修订号和状态对不上、首建新章时上一章事务还没提交，都该拦下。但这套逻辑只进了共享 JS 核与 Codex 的 Python 实现，Claude Code 实际挂的是 bash 守卫，一条都没有。结果是同一个工程、同一次写正文，OpenCode / ZCode / Codex 拦下并给出修复动作，Claude Code 直接放行，一路写到 SessionStart 提醒或批末校验才暴露，那时只能走 `/story-import` 补救。现在 bash 守卫调共享核做同一份判定，四端一致。追踪检查要解析 JSON、只能靠 node，因此 node 不在场时这道门放行（大纲/细纲那道门仍是纯 bash，无 node 也拦得住）。

- **正文里的普通「说」不再被判违规（#312）**。`.claude/rules/story-format.md` 是全仓唯一禁止「他说」「她道」的地方，与另外 11 处写法冲突，而它恰好在 `正文/` 路径上加载。改成只治高频和公式化的对话标签，普通「说」低频使用照常。同一批还删掉了只在 `拆文库/` `对标/` `设定/` 三个路径下加载、正文根本命中不到的「禁止 AI 腔」红线块——那五条规则已由 narrative-writer 的 7 Gate 与 `check-ai-patterns.js` 的 blocking 规则覆盖。

### 优化

- **长篇写作的无条件加载降三成（#269）**。`story-long-write/SKILL.md` 是这个 skill 唯一无条件整份进上下文的文件，此前 82 KB / 698 行，其中开书三阶段（选题方向、核心设定、大纲搭建）占了 30 KB。这三段每本书只跑一次，日更和单章写作都用不到，却每次会话都付。现在抽进 `references/workflow-setup.md` 按需读，SKILL.md 降到 54 KB / 456 行，降幅 34%。留在 SKILL.md 的是场景路由、停靠规则、Phase 4 单章写作与 Phase 5 检查——即每章都要用的部分，一个字没删。「对标书路径查找」这类被 Phase 4 反复引用的定义提到公共小节，两边共用一条规则。

- **narrative-writer 模板精简约 19%（#312）**。每章 spawn 一次、常驻上下文的那份模板从 36.5 KB 降到 29.5 KB：删掉与 7 Gate、禁止事项重复的审查清单（`/story-review` spawn 时会内联完整评分标准），以及 `……`／`——`、段间空行、章节元信息正则的重复陈述——这些每章由归一化脚本机械处理，写作规则本身没有放宽，Gate A-G 与禁止事项口径不变。

### 维护

- 写正文守卫的 bash 那一面纳入 parity 断言。`test-prose-net-parity.sh` 此前只锁 Codex Python 与 JS 核，bash 侧没有任何跨端断言，#283 漏掉 Claude Code 整整一版正是因为这个空档。新增的 Part F 按「同一工程同一次写入，bash 拦不拦等于 JS 核拦不拦」逐场景比对 10 组工程状态，并锚死每个场景的期望方向（否则两端一起漏拦也能比对干净），另含 node 缺席必须放行的降级断言。
