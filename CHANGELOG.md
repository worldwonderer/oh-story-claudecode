...
- **Claude Code 上写正文补回追踪检查（#305）**。v0.7.3 给写正文守卫加了四道硬阻断：追踪状态文件缺失、schema 不是当前版本、续写状态卡的修订号与状态对不上、首建新章时上一章事务还没提交。这套判定当时只进了共享 JS 核与 Codex 的 Python 实现，而 Claude Code 挂的是另一份纯 bash 守卫，四条一条都没有。于是同一个工程、同一次写正文，OpenCode / ZCode / Codex 拦下并给出修复动作，Claude Code 直接放行，要等到会话起点的连续性提醒或批末校验才发现，那时只能重新导入补救。现在 bash 守卫改调同一份共享核，四端判定一致、提示一致，续写和改稿也一并纳入判定。这道检查要解析 JSON、只能靠 node，机器上没有 node 时它不生效，细纲那道门则仍是纯 bash，任何情况下都拦得住。

- **正文里的普通「他说」不再被判违规（#312）**。`story-format.md` 把「他说」「她道」列为绝对禁止，是全仓唯一把普通「说」判成违规的地方，与 `format-and-structure.md` 等 11 处「普通说可保留」的口径冲突，而它的加载路径正好是 `正文/`，写正文时必定生效。现在改为「避免对话标签机械化」：高频和公式化的标签用动作或上下文替代，普通「说」低频使用可以保留。同批还删掉了 `story-narrative.md` 的「禁止 AI 腔」红线块——它只在 `拆文库/`、`对标/`、`设定/` 下加载，正文目录根本不命中，那五条规则也已由 narrative-writer 的 7 Gate 和确定性检测脚本覆盖。
...
- **长篇写作每次触发的固定加载少掉三分之一（#269）**。`story-long-write` 的 SKILL.md 是这个 skill 唯一无条件整份进上下文的文件，`references/` 下那几十个文件都走「参考资料索引」按需读取。它上一版 699 行，其中开书三阶段（确认选题方向、核心设定、大纲搭建）占了 244 行——这三段每本书只跑一次，日更续写和单章写作都用不上，却每次触发都要加载。现在移进 `references/workflow-setup.md`，要开书时才读，SKILL.md 降到 455 行。留下的是每章都要用的部分：场景路由、裸调用与停靠点、单章写作流程、质量检查、参考资料索引。移动前后按字节比对过，单章写作与质量检查两节逐字未动。
